...
class BaseGameMode(procgame.game.AdvancedMode):
    ...
    def evt_ball_ending(self, shoot_again, last_ball):
        ...
        """
        self.game.log(
            "base game mode trough changed notification ('ball_ending - again=%s, last=%s')"
            % (shoot_again, last_ball)
        )
    ...
    def sw_outlaneL_active(self, sw):
        if self.game.getPlayerState("kickbackEnabled"):
            ...
            self.delay(
                name="disable_kickback", delay=3.0, handler=self.kickback_disabler
            )
        else:
    ...
    def checkAllSwitches(self):
        ...
        if (
            (self.game.getPlayerState("standupSwitchL") == True)
            and (self.game.getPlayerState("standupSwitchC") == True)
            and (self.game.getPlayerState("standupSwitchR") == True)
        ):  # all three are True
