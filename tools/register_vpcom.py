...
class Controller:
    ...
    _public_methods_ = ["Run", "Stop", "PrintGlobal"]
    ...
    def Run(self):
        ...
        vp_game_map_file = config.value_for_key_path(
            keypath="vp_game_map_file", default="/."
        )
    ...
    def SetSwitch(self, number, value):
        ...
        if not self.game.switches.has_key(prNumber):
            return False
    ...
    def AddSwitchEvent(self, prNumber, value):
        ...
        if value:
            self.game.proc.add_switch_event(
                prNumber, pinproc.EventTypeSwitchClosedNondebounced
            )
            self.game.proc.add_switch_event(
                prNumber, pinproc.EventTypeSwitchClosedDebounced
            )
        else:
            self.game.proc.add_switch_event(
                prNumber, pinproc.EventTypeSwitchOpenNondebounced
            )
            self.game.proc.add_switch_event(
                prNumber, pinproc.EventTypeSwitchOpenDebounced
            )
    ...
    def getCoilStates(self):
        ...
        for i in range(len(vpcoils)):
            ...
            # since they stay on until the button is released
                ...
                vpcoils[i] = pycoils[
                    pinproc.decode(self.game.machine_type, "FURH")
                ].curr_state
            elif i == 36:
                vpcoils[i] = pycoils[
                    pinproc.decode(self.game.machine_type, "FULH")
                ].curr_state
            elif i < 44:
                ...
                vpcoils[i] = pycoils[
                    pinproc.decode(self.game.machine_type, "FLRH")
                ].curr_state
            elif i == 48:
                vpcoils[i] = pycoils[
                    pinproc.decode(self.game.machine_type, "FLLH")
                ].curr_state
            else:
