...
class Attract(game.Mode):
    ...
    def __init__(self, game):
        ...
        self.proc_banner = dmd.TextLayer(128 / 2, 7, font_jazz18, "center").set_text(
            "pyprocgame"
        )
        self.splash = dmd.FrameLayer(
            opaque=True, frame=dmd.Animation().load(fonts_path + "Splash.dmd").frames[0]
        )
        self.layer = dmd.ScriptedLayer(
            128,
            32,
            [
                {"seconds": 2.0, "layer": self.splash},
    ...
    def mode_started(self):
        ...
        for i in range(0, 32):
            ...
            if i > 16:
                lamp_schedules[i] = (
                    lamp_schedules[i] | (0xFFFF << (32 - (i - 16)))
                ) & 0xFFFFFFFF
        ...
        for lamp in self.game.lamps:
            if (
                lamp.name.find("gi0", 0) == -1
                and lamp.name != "startButton"
                and lamp.name != "buyIn"
                and lamp.name != "superGame"
            ):
                lamp.schedule(
                    schedule=lamp_schedules[i % 32], cycle_seconds=0, now=False
                )
                i += 1
