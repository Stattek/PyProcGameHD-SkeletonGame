...
try:
    ...
    print(
        "Error importing Mac OS X PyObjC frameworks.  This application requires Mac OS X."
    )
    sys.exit(1)
...
def dmd_frame_as_nsimage(frame):
    ...
    image = NSImage.alloc().initWithSize_(
        NSMakeSize(frame.width * dot_size, frame.height * dot_size)
    )
    image.lockFocus()
...
def dmd2mov(input_path, output_path, fps):
    ...
    for frame in anim.frames:
        ...
        movie.addImage_forDuration_withAttributes_(
            image, QTMakeTimeWithTimeInterval(1.0 / float(fps)), attrs
        )
