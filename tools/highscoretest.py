...
class Attract(game.Mode):
    def mode_started(self):
        ...
        for frame in highscore.generate_highscore_frames(
...
class TestGame(game.BasicGame):
    ...
    def setup(self):
        ...
        cat.scores = [
            highscore.HighScore(score=5000000, inits="GSS"),
    ...
    def highscore_entry_ready_to_prompt(self, mode, prompt):
        ...
        text = "[Great Score]\n#%s#" % (
            prompt.left.upper()
        )  # we know that the left is the player name
        frame = markup.frame_for_markup(markup=text, y_offset=0)
        frame_layer = dmd.FrameLayer(frame=frame)
        frame_layer.blink_frames = 10
        banner_mode.layer = dmd.ScriptedLayer(
            width=128, height=32, script=[{"seconds": 3.0, "layer": frame_layer}]
        )
        banner_mode.layer.on_complete = lambda: self.highscore_banner_complete(
            banner_mode=banner_mode, highscore_entry_mode=mode
        )
