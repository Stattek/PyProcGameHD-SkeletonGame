...
class Desktop(object):
    ...
    def setup_window(self):
        self.window = pyglet.window.Window(
            width=DMD_SIZE[0] * DMD_SCALE, height=DMD_SIZE[1] * DMD_SCALE
        )
        ...
        def on_key_press(symbol, modifiers):
            if (
                symbol == pyglet.window.key.C and modifiers & pyglet.window.key.MOD_CTRL
            ) or (symbol == pyglet.window.key.ESCAPE):
                self.append_exit_event()
            elif symbol in self.key_map:
                self.key_events.append(
                    {
                        "type": pinproc.EventTypeSwitchClosedDebounced,
                        "value": self.key_map[symbol],
                    }
                )
...
class FrameDrawer(object):
    ...
    def draw(self, frame):
        ...
        image = pyglet.image.ImageData(
            DMD_SIZE[0], DMD_SIZE[1], "RGB", data, pitch=DMD_SIZE[0] * 3
        )

        gl.glTexParameteri(
            image.get_texture().target, gl.GL_TEXTURE_MAG_FILTER, gl.GL_NEAREST
        )
        ...
        self.mask_texture.blit_tiled(
            x=0, y=0, z=0, width=DMD_SIZE[0] * MASK_SIZE, height=DMD_SIZE[1] * MASK_SIZE
        )
