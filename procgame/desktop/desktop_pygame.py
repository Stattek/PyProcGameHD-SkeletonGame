...
class Desktop:
    ...
    def __init__(self):
        # TODO: are these type annotations right?
        ...
        if "pygame" in globals():
            ...
            self.screen_position_x: int = config.value_for_key_path(
                keypath="screen_position_x", default=0
            )
            self.screen_position_y: int = config.value_for_key_path(
                keypath="screen_position_y", default=0
            )
            self.screen_scale = config.value_for_key_path(
                keypath="desktop_dmd_scale", default=2
            )
            self.dot_filter: bool = config.value_for_key_path(
                keypath="dmd_dot_filter", default=True
            )
            self.fullscreen: bool = config.value_for_key_path(
                keypath="dmd_fullscreen", default=False
            )
            self.window_border: bool = config.value_for_key_path(
                keypath="dmd_window_border", default=True
            )
            self.dmd_screen_size: int = (
            ...
            if self.dot_filter:
                ...
                self.grid_image = pygame.surface.Surface(
                    (self.dots_w * 10, self.dots_h * 10), pygame.SRCALPHA
                )
                r = pygame.Rect(0, 0, self.dots_w * 10, self.dots_h * 10)
                self.grid_image.fill((0, 0, 0, 0), r)

                grid_32x32segment = pygame.image.load(
                    dmd_grid_path + "dmdgrid32x32.png"
                )
                ...
                self.grid_image = pygame.transform.smoothscale(
                    self.grid_image, self.dmd_screen_size
                ).convert_alpha()
            else:
                self.draw = self.draw_no_dot_effect
        else:
            print(
                "Desktop init skipping setup_window(); pygame does not appear to be loaded."
            )
    ...
    def get_keyboard_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.locals.KEYDOWN:
                if (
                    event.key == pygame.locals.K_RCTRL
                    or event.key == pygame.locals.K_LCTRL
                ):
                    self.ctrl = 1
    ...
    def setup_window(self):
        ...
        self.scratch_surface = pygame.surface.Surface(
