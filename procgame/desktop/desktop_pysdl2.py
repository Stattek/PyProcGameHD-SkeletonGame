...
class Desktop:
    ...
    dots_h = 32
    screen_scale = (
        2  # this is the factor to pygame scale the display.  192x96 (x6) = 1152x576
    )
    ...
    def __init__(self):
        ...
        self.screen_position_x = config.value_for_key_path(
            keypath="screen_position_x", default=0
        )
        self.screen_position_y = config.value_for_key_path(
            keypath="screen_position_y", default=0
        )
        self.screen_scale = config.value_for_key_path(
            keypath="desktop_dmd_scale", default=2
        )
        self.dot_filter = config.value_for_key_path(
            keypath="dmd_dot_filter", default=True
        )
        self.fullscreen = config.value_for_key_path(
            keypath="dmd_fullscreen", default=False
        )
        self.window_border = config.value_for_key_path(
            keypath="dmd_window_border", default=True
        )
        self.dmd_screen_size = (
            (self.dots_w) * self.screen_scale,
            (self.dots_h) * self.screen_scale,
        )
        self.dmd_soften = config.value_for_key_path(keypath="dmd_soften", default="0")
        self.use_rgb_dmd_device = config.value_for_key_path(
            keypath="rgb_dmd.enabled", default=False
        )
        if self.use_rgb_dmd_device:
            ...
            self.serial_port_number = config.value_for_key_path(
                keypath="rgb_dmd.com_port", default=None
            )
            if self.serial_port_number is None:
                raise ValueError(
                    "RGBDMD: config.yaml specified rgb_dmd enabled, but no com_port value (e.g., com3) given!"
                )
        ...
        if self.use_rgb_dmd_device:
            if serial is None:
                raise ValueError(
                    "RGBDMD: config.yaml specified rgb_dmd enabled, but requird pySerial library not installed/found."
                )
            self.serialPort = serial.Serial(
                port=self.serial_port_number, baudrate=2500000
            )
            self.magic_cookie = bytearray(
                [0xBA, 0x11, 0x00, 0x03, 0x04, 0x00, 0x00, 0x00]
            )
        ...
        if self.dot_filter == True:
            dmd_grid_path = config.value_for_key_path(
                keypath="dmd_grid_path", default="./"
            )
            # self.dot_filter = False
            # self.draw = self.draw_no_dot_effect

            ############## Make the Dot filter ############################
            dot_sprite = sdl2_DisplayManager.inst().load_texture(
                os.path.join(dmd_grid_path, "dmdgrid32x32.png")
            )

            # 1. Make the destination texture (huge)
            self.dot_tex = sdl2.render.SDL_CreateTexture(
                sdl2_DisplayManager.inst().texture_renderer.renderer,
            ...
            bk = sdl2.SDL_GetRenderTarget(
                sdl2_DisplayManager.inst().texture_renderer.renderer
            )
            sdl2.SDL_SetRenderTarget(
                sdl2_DisplayManager.inst().texture_renderer.renderer, self.dot_tex
            )
            ...
            sdl2.SDL_SetRenderTarget(
                sdl2_DisplayManager.inst().texture_renderer.renderer, bk
            )  # revert back
        else:
            self.draw = self.draw_no_dot_effect
        ...
        for event in sdl2.ext.get_events():
            ...
            if event.type == sdl2.SDL_KEYDOWN:
                if (
                    ...
                    or event.key.keysym.sym == sdl2.SDLK_RCTRL
                ):
                    self.ctrl = 1
    ...
    def setup_window(self):
        ...
        sdl2_DisplayManager.Init(
            self.dots_w,
            self.dots_h,
            self.screen_scale,
            "PyProcGameHD.  [CTRL-C to exit]",
    ...
    def draw(self, frame):
        ...
        if not self.fullscreen == True:
            sdl2_DisplayManager.inst().screen_blit(
                source_tx=frame.pySurface, expand_to_fill=True
            )
        else:
            sdl2.SDL_RenderCopy(
                sdl2_DisplayManager.inst().texture_renderer.renderer,
                frame.pySurface.texture,
                None,
                sdl2.rect.SDL_Rect(0, 0, self.window_w, self.window_h),
            )
        ...
        if not self.fullscreen == True:
            ...
            sdl2.SDL_RenderCopy(
                ...
                sdl2.rect.SDL_Rect(
                    self.screen_position_x,
                    self.screen_position_y,
                    self.screen_position_x + self.window_w,
                    self.screen_position_y + self.window_h,
                ),
            )
        sdl2_DisplayManager.inst().flip()
    ...
    def draw_to_rgb_dmd(self, frame):
        ...
        bucket = sdl2_DisplayManager.inst().make_bits_from_texture(
            frame.pySurface, 128, 32
        )
