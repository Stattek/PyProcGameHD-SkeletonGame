...
class HighScoreCategory:
    ...
    def load_from_game(self, game):
        ...
        if self.game_data_key in game.game_data:
            ...
            game.logger.warning(
                "HighScoreCategory.load_from_game(): game_data_key %s not found in game_data.",
                self.game_data_key,
            )
...
class CategoryDrivenDataHelper:
    ...
    def add_placeholder(self, category, score, name):
        ...
        category.scores = sorted(
            category.scores, reverse=True
        )  # Reverse to sort from high to low.
            ...
            for index, score in enumerate(category.scores):
                if score.inits is None:
                    ...
                    if score.key in keyed_prompts:
                        ...
                        keyed_prompts[score.key] = EntryPrompt(
                            left=score.name, right=[new_title]
                        )
    ...
    def prompts(self):
        ...
        for category in self.categories:
            for player in self.game.players:
                self.data.add_placeholder(
                    category=category,
                    score=category.score_for_player(player),
