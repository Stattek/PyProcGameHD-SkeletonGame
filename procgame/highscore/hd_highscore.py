...
class HD_InitialEntryMode_ML(Mode):
    ...
    *left_text* and *right_text* are strings or arrays to be displayed at the
    ...
    def __init__(self, game, priority, left_text, right_text, entered_handler):
        ...
        for text in left_text:
            ...
            for w in words:
                ...
                i += 1
            script.append(
                {"seconds": seconds_per_text, "layer": dmd.FrameLayer(frame=frame)}
            )
        topthird_left_layer = dmd.ScriptedLayer(
            width=self.display_width, height=mh, script=script
        )
        ...
        for text in right_text:
            ...
            for w in words:
                self.text_font.draw(
                    frame,
                    w,
                    self.display_width - (self.text_font.size(w)[0]),
        ...
        topthird_right_layer = dmd.ScriptedLayer(
            width=self.display_width, height=mh, script=script
        )
        ...
        self.inits_layer = dmd.HDTextLayer(
        ...
        fbox.fill_rect(
            0, 0, self.space_per_char + 2, self.space_per_char + 2, (128, 128, 255)
        )
        fbox.fill_rect(
            2, 2, self.space_per_char - 4 + 2, self.space_per_char - 4 + 2, (0, 64, 128)
        )
        ...
        self.char_opts_layer.set_target_position(
            (
                self.display_width
                - (self.columns_of_chars_in_palette + 1) * self.space_per_char
            )
            / 2,
    ...
    def animate_to_index(self, new_index, inc=0):
        ...
        self.selection_box_layer.set_target_position(
            x * self.space_per_char + bx + self.space_per_char / 2,
            y * self.space_per_char + by + self.space_per_char / 2,
        )
        print(
            "moving box to:",
            self.selection_box_layer.target_x,
            ",",
            self.selection_box_layer.target_y,
        )
        ...
        if letter == self.char_back:
            ...
            if self.entered_handler != None:
                ...
                self.game.logger.warning(
                    "InitialEntryMode finished but no entered_handler to notify!"
                )
        else:
            self.inits += letter
    ...
    def periodic_left(self):
        ...
        self.delay(
            name="periodic_movement",
            event_type=None,
            delay=0.2,
            handler=self.periodic_left,
        )
...
class HD_InitialEntryMode_ML_old(Mode):
    ...
    *left_text* and *right_text* are strings or arrays to be displayed at the
    ...
    def __init__(self, game, priority, left_text, right_text, entered_handler):
        ...
        topthird_left_layer = dmd.ScriptedLayer(
            width=480, height=self.text_font_height, script=script
        )
        topthird_left_layer.composite_op = "blacksrc"
        self.layer.layers += [topthird_left_layer]
    ...
    def animate_to_index(self, new_index, inc=0):
        ...
        for x in rng:
            ...
            for offset in range(-7, 8):
                ...
                self.letters_font.draw(
                    frame,
                    self.letters[index],
                    450 / 2 - offset * letter_spread - letter_width / 2 + x,
                    0,
                )
        ...
        for x in range(len(self.inits)):
            self.init_font.draw(
                self.inits_frame, self.inits[x], x * init_spread + x_offset, 0
            )
