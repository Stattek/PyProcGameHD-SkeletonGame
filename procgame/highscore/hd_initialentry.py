...
class HD_InitialEntryMode(Mode):
    ...
    *left_text* and *right_text* are strings or arrays to be displayed at the
    ...
    def __init__(self, game, priority, left_text, right_text, entered_handler):
        ...
        self.init_font = self.game.fonts[
            "high_score_entry_inits"
        ]  # dmd.hdfont_named('consolas', 12)
        self.text_font = self.game.fonts[
            "high_score_entry_msg"
        ]  # dmd.hdfont_named('consolas', 12)
        self.letters_font = self.game.fonts[
        ...
        topthird_left_layer = dmd.ScriptedLayer(
            width=self.game.dmd.width, height=self.text_font_height, script=script
        )
        ...
        for text in right_text:
            frame = dmd.Frame(width=self.game.dmd.width, height=self.text_font_height)
            self.text_font.draw(
                frame, text, self.game.dmd.width - (self.text_font.size(text)[0]), 0
            )
            script.append(
                {"seconds": seconds_per_text, "layer": dmd.FrameLayer(frame=frame)}
            )
        ...
        self.inits_entry_layer = dmd.HDTextLayer(
            self.game.dmd.width / 2,
            self.text_font_height,
            self.init_font,
            "center",
            vert_justify="top",
    ...
    def animate_to_index(self, new_index, inc=0):
        ...
        for x in rng:
            frame = dmd.Frame(
                width=self.game.dmd.width, height=self.init_font_height + 2
            )
            for offset in range(-7, 8):
                ...
                self.letters_font.draw(
                    ...
                    self.letters[index],
                    self.game.dmd.width / 2
                    - offset * letter_spread
                    - letter_width / 2
                    + x,
                    0,
                )
            frame.fill_rect(
                self.game.dmd.width / 2 - letter_width / 2 - 4,
                0,
                2,
                self.letters_font_height + 2,
                (255, 255, 0, 255),
            )
            frame.fill_rect(
                self.game.dmd.width / 2 + letter_width / 2,
                0,
                2,
                self.letters_font_height + 2,
                (255, 255, 0, 255),
...
class HD_EntrySequenceManager(EntrySequenceManager):
    ...
    def create_highscore_entry_mode(self, left_text, right_text, entered_handler):
        ...
        if self.use_multiline:
            return HD_InitialEntryMode_ML(
                game=self.game,
                priority=self.priority + 1,
                left_text=left_text,
                right_text=right_text,
                entered_handler=entered_handler,
            )
        else:
            return HD_InitialEntryMode(
                game=self.game,
                priority=self.priority + 1,
                left_text=left_text,
                right_text=right_text,
