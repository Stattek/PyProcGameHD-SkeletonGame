...
class DMDHelper(Mode):
    ...
    def __init__(self, game):
        ...
        if (
            "status_bg" not in self.game.animations
            or self.game.animations["status_bg"] is None
        ):
            t2 = dmd.SolidLayer(
                int(self.game.dmd_width * 0.8),
                int(self.game.dmd_height * 0.5),
                (255, 196, 0, 255),
            )
            self.game.animations["status_bg"] = dmd.GroupedLayer(
                int(self.game.dmd_width * 0.8), int(self.game.dmd_height * 0.5), [t2]
            )  # ,t1
    ...
    def genMsgFrame(
        ...
        if isinstance(msg, list):
            ...
            for line in msg:
                if line != "":
                    ...
                    tL = dmd.HDTextLayer(
                        ...
                        self.game.dmd.height * i / (num_lines + 1),
                        font,
                        "center",
                        vert_justify="center",
                        opaque=False,
                        ...
                        height=100,
                        line_color=font_style.line_color,
                        line_width=font_style.line_width,
                        interior_color=font_style.interior_color,
                        fill_color=font_style.fill_color,
                    ).set_text(line, blink_frames=flashing)
                    t_layers.append(tL)
                i = i + 1
            ...
            t = dmd.HDTextLayer(
                ...
                interior_color=font_style.interior_color,
                fill_color=font_style.fill_color,
            ).set_text(msg, blink_frames=flashing)
        ...
        if background_layer is None:
            ...
            if isinstance(background_layer, list):
                lAnimations = [
                    self.game.animations[animation_key]
                    for animation_key in background_layer
                ]
                ...
                gl = dmd.GroupedLayer(
                    self.game.dmd.width, self.game.dmd.height, lAnimations
                )
            else:
    ...
    def showMessage(
        ...
        # create a new timer
        self.delay(
            name=self.timer_name, event_type=None, delay=duration, handler=self.msg_over
        )

    def __parse_relative_num(
        self, yaml_struct, key, relative_to, default, relative_match=None
    ):
    ...
    def __parse_position_data(self, yaml_struct, for_text=False):
        ...
        x = self.__parse_relative_num(
            yaml_struct,
            key="x",
            relative_to=self.game.dmd.width,
            default=fallback,
            relative_match="width",
        )
        y = self.__parse_relative_num(
            yaml_struct,
            key="y",
            relative_to=self.game.dmd.height,
            default=fallback,
    ...
    def parse_font_data(self, yaml_struct, required=True):
        ...
        if fname is None:
            if required:
                raise ValueError(
                    "Cannot find required Font tag in yaml segment [%s]" % (yaml_struct)
                )
            else:
                f = None
        else:
            if fname not in self.game.fonts:
                self.logger.error(
                    "yaml refers to a font '%s' that does not exist.  Please check the assetList to ensure this font is present"
                    % fname
                )
        ...
        # get font style
        font_style = value_for_key(
            yaml_struct, "font_style", value_for_key(yaml_struct, "FontStyle")
        )
        if isinstance(font_style, dict):
            ...
            font_style = dmd.HDFontStyle(
                interior_color=ic, line_width=lw, line_color=lc
            )
        ...
        try:
            if "HighScores" in yamlStruct:
                ...
                background = value_for_key(
                    v, "Background", value_for_key(v, "Animation")
                )
                ...
                lyrTmp = dmd.ScoresLayer(
                    self.game, fields, fnt, font_style, background, duration
                )
                ...
                if multiple_screens:
                    lyrTmp = dmd.ScriptlessLayer(
                        self.game.dmd.width, self.game.dmd.height
                    )
                    lyrTmp.append(
                        self.genMsgFrame(
                            ["Last Game", "Final Scores"],
                            background,
                            font_key=fnt,
                            font_style=font_style,
                        ),
                    ...
                    for player in self.game.old_players:
                        lT = self.genMsgFrame(
                            [
                                player.name,
                                self.game.score_display.format_score(player.score),
                            ],
                            background,
                            font_key=fnt,
                            font_style=font_style,
                ...
                for line in randomText:
                    ...
                    if headerText is not None:
                        completeText[:0] = [
                            headerText
                        ]  # prepend the header text entry at the start of the list

                    rndmLayers.append(
                        self.genMsgFrame(
                            completeText,
                            value_for_key(v, "Animation"),
                            font_key=fnt,
                            font_style=font_style,
                        )
                    )
                ...
                lyrTmp = self.generateLayerFromYaml(
                    yamlStruct
                )  # not sure what this is, let the other method parse it
                v = yamlStruct[
                    yamlStruct.keys()[0]
                ]  # but we reach in and grab it to pull duration, lampshow and sound.
    ...
    def generateLayerFromYaml(self, yaml_struct):
        ...
        if yaml_struct is None or (
            isinstance(yaml_struct, basestring) and yaml_struct == "None"
        ):
            return None
        ...
        try:
            if "display" in yaml_struct:
                ...
                new_layer = self.game.animations[
                    value_for_key(
                        v, "Name", value_for_key(v, "Animation"), exception_on_miss=True
                    )
                ]
                new_layer.reset()

                if (
                    value_for_key(v, "duration") is None
                ):  # no value found, set it so it will be later.
                    v["duration"] = new_layer.duration()

            elif "sequence_layer" in yaml_struct:
                v = yaml_struct["sequence_layer"]

                new_layer = dmd.ScriptlessLayer(
                    self.game.dmd.width, self.game.dmd.height
                )
                ...
                for c in v["contents"]:
                    if not "item" in c:
                        raise ValueError(
                            "malformed YAML file; sequence must contain a list of 'item's"
                        )
                    c = c["item"]
                ...
                c = self.generateLayerFromYaml(
                    value_for_key(v, "contents", exception_on_miss=True)
                )

                new_layer = dmd.PanningLayer(
                    width=w,
                    height=h,
                    frame=c,
                    origin=(origin_x, origin_y),
                    translate=(scroll_x, scroll_y),
                    numFramesDrawnBetweenMovementUpdate=frames_per_movement,
                    bounce=bounce,
                )
                ...
                source_layer = self.game.animations[
                    value_for_key(
                        v, "name", value_for_key(v, "Name"), exception_on_miss=True
                    )
                ]
                ...
                if len(frame_list) == 0:
                    ...
                    new_layer = AnimatedLayer(
                        frame_time=source_layer.frame_time,
                ...
                w = self.__parse_relative_num(
                    v, "width", self.game.dmd.width, default=None
                )
                h = self.__parse_relative_num(
                    v, "height", self.game.dmd.height, default=None
                )
                ...
                w = self.__parse_relative_num(v, "width", self.game.dmd.width, None)
                bold_font, bold_style = self.parse_font_data(
                    value_for_key(v, "Bold", exception_on_miss=True)
                )
                plain_font, plain_style = self.parse_font_data(
                    value_for_key(v, "Normal", exception_on_miss=True)
                )
                ...
                gen = dmd.MarkupFrameGenerator(
                    game=self.game, font_plain=plain_font, font_bold=bold_font, width=w
                )
                gen.set_bold_font(
                    bold_font,
                    interior_color=bold_style.interior_color,
                    border_width=bold_style.line_width,
                    border_color=bold_style.line_color,
                )
                gen.set_plain_font(
                    plain_font,
                    interior_color=plain_style.interior_color,
                    border_width=plain_style.line_width,
                    border_color=plain_style.line_color,
                )
                ...
                raise ValueError(
                    "Unknown tag '%s' in yaml section.  Check spelling/caps/etc."
                    % (unknown_tag)
                )
