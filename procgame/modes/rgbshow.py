...
class RgbShowPlayer(AdvancedMode):
    ...
    def play_show(self, key, repeat=None, callback=None, save_state=True):
        ...
        if key in self.active_shows:
            self.logger.info(
                "suppressing request to play already active show: %s" % key
            )
            return
    ...
    def save_state(self, key):
        ...
        if key not in self.shows:
            self.logger.info(
                "suppressing request to save_state for unknown show: %s" % key
            )
            return
    ...
    def restore_state(self, key):
        ...
        if key not in self.shows:
            self.logger.info(
                "suppressing request to restore_state for unknown show: %s" % key
            )
            return
        device_list = self.shows[key].get_device_list()
    ...
    def update(self, now):
        ...
        if self.enabled:
            if now >= len(self.data):
                raise ValueError(
                    "Track '%s' received index '%d' beyond the length of the track (%d)"
                    % (self.name, now, self.length)
                )
            cmd = self.data[now]
            if cmd is not None:
                cmd.process_command()
    ...
    def __init__(self, line, color_map, show):
        ...
        if device_type is None:
            ...
            if self.name in show.game.leds:
                ...
                raise ValueError(
                    "RGB Track created for unknown device named '%s'" % self.name
                )
        ...
        for i in range(0, len(data), 1):
            ...
            if this_color != last_color:
                ...
                if last_color is not None:
                    ...
                    if cdata is None:
                        ...
                        c = RgbCommand(
                            self.name, fn, cdata["color"], last_run_length * show.time
                        )
                    else:
            ...
            if i == len(data) - 1:  # last slot
                if (last_run_length == 0) or (
                    last_color == this_color
                ):  # just started a new run; so store this run
