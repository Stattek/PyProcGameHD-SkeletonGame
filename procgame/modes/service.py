...
class ServiceModeSkeleton(Mode):
    ...
    def __init__(self, game, priority, font):
        ...
        self.layer = GroupedLayer(
            128, 32, [self.title_layer, self.item_layer, self.instruction_layer]
        )
    ...
    def __init__(self, game, priority, font, extra_tests=[]):
        ...
        if len(self.game.settings) > 0:
            self.settings = Settings(
                self.game, self.priority + 1, font, "Settings", self.game.settings
            )
    ...
    def mode_started(self):
        ...
        if self.game.lamps.has_key("startButton"):
            self.game.lamps.startButton.schedule(
                schedule=0xFF00FF00, cycle_seconds=0, now=False
            )
...
class Statistics(ServiceModeList):
    ...
    def __init__(self, game, priority, font, name, itemlist):
        ...
        for section in itemlist:
            self.items.append(
                StatsDisplay(
                    self.game, priority + 1, font, str(section), itemlist[section]
                )
            )
    ...
    def __init__(self, game, priority, font, name, itemlist):
        ...
        for item in sorted(itemlist.iterkeys()):
            if type(itemlist[item]) == type({}):
                self.items.append(
                    HighScoreItem(
                        str(item), itemlist[item]["name"], itemlist[item]["score"]
                    )
                )
            else:
...
class SettingsEditor(ServiceModeList):
    ...
    def __init__(self, game, priority, font, name, itemlist):
        ...
        for item in sorted(itemlist.iterkeys()):
            ...
            if "increments" in itemlist[item]:
                ...
                self.items.append(
                    EditItem(
                        str(item), option_list, self.game.user_settings[self.name][item]
                    )
                )
            else:
                self.items.append(
    ...
    def process_enter(self):
        if self.state == "nav":
            ...
            self.delay(name="blink", event_type=None, delay=0.3, handler=self.blinker)
        else:
            self.state = "nav"
