...
class Attract(Mode):
    ...
    def __init__(self, game, yaml_file="config/attract.yaml", start_button_lamp=None):
        super().__init__(game=game, priority=9)
        ...
        try:
            ...
            self.game.log(
                "Attract mode: Error loading yaml file from %s; the file has a syntax error in it!\nDetails: %s"
                % (yaml_file, e)
            )
            raise
        except Exception, e:
            self.game.log(
                "Attract mode: Error loading yaml file from %s: %s" % (yaml_file, e)
            )
            raise
    ...
    def next_sound(self):
        ...
        if sound_key in self.game.sound.music:
            if (
                self.game.user_settings["Machine (Standard)"]["Attract Mode Music"]
                == "On"
            ):
                self.game.sound.play_music(sound_key)
        else:
            if (
                self.game.user_settings["Machine (Standard)"]["Attract Mode Sounds"]
                == "On"
            ):
    ...
    def mode_started(self):
        self.reset()
        if self.start_button_lamp is not None:
            self.start_button_lamp.schedule(
                schedule=0xFFFF0000, cycle_seconds=0, now=False
            )
