...
class ScoreDisplayHD(Mode):
    ...
    def __init__(self, game, priority, yaml_file_path="config/new_score_display.yaml"):
        ...
        try:
            ...
            self.game.log(
                "score_display: Error loading yaml file from %s: %s"
                % (yaml_file_path, e)
            )
            raise
        ...
        self.single_player["bg"] = self.game.dmdHelper.generateLayerFromYaml(
            value_for_key(values, "ScoreLayout.SinglePlayer.Background")
        )

        self.single_player_layers.append(self.single_player["bg"])

        self.single_player["credit_layer"] = (
            self.game.dmdHelper.generateTextLayerFromYaml(
                value_for_key(values, "ScoreLayout.SinglePlayer.CreditIndicator")
            )
        )
        self.single_player_credit_template = value_for_key(
            values, "ScoreLayout.SinglePlayer.CreditIndicator.format", "FREE PLAY"
        )
        ...
        self.single_player["ball_number"] = (
            self.game.dmdHelper.generateTextLayerFromYaml(
                value_for_key(values, "ScoreLayout.SinglePlayer.BallNumber")
            )
        )
        self.single_player_ball_number_template = value_for_key(
            values, "ScoreLayout.SinglePlayer.BallNumber.format", "BALL _"
        )

        if "_" in self.single_player_ball_number_template:
            self.single_player["ball_number"].set_text(
                self.single_player_ball_number_template.replace(
                    "_", "%d" % self.game.ball
                )
            )
        ...
        self.single_player["score"] = self.game.dmdHelper.generateTextLayerFromYaml(
            value_for_key(values, "ScoreLayout.SinglePlayer.Score")
        )
        self.single_player_layers.append(self.single_player["score"])

        self.single_player["fg"] = self.game.dmdHelper.generateLayerFromYaml(
            value_for_key(values, "ScoreLayout.SinglePlayer.Foreground")
        )
        ...
        multiplayer_bg_struct = value_for_key(
            values, "ScoreLayout.MultiPlayer.Background", "Same"
        )
        if multiplayer_bg_struct == "Same":
            self.multiplayer["bg"] = self.single_player["bg"]
        else:
            # could be different or 'None'
            self.multiplayer["bg"] = self.game.dmdHelper.generateLayerFromYaml(
                multiplayer_bg_struct
            )

        self.multiplayer_layers.append(self.multiplayer["bg"])

        multiplayer_fg_struct = value_for_key(
            values, "ScoreLayout.MultiPlayer.Foreground", "Same"
        )
        if multiplayer_fg_struct == "Same":
            self.multiplayer["fg"] = self.single_player["fg"]
        else:
            # otherwise, set from explicit values
            self.multiplayer["fg"] = self.game.dmdHelper.generateLayerFromYaml(
                multiplayer_fg_struct
            )

        # Make the Multiplayer Credit Indicator TextLayer - if omitted, we use the single player credit indicator and location
        credit_indicator_data = value_for_key(
            values, "ScoreLayout.MultiPlayer.CreditIndicator"
        )
        if credit_indicator_data is None:
            self.multiplayer["credit_layer"] = self.single_player["credit_layer"]
        else:
            self.multiplayer["credit_layer"] = (
                self.game.dmdHelper.generateTextLayerFromYaml(credit_indicator_data)
            )
            self.multiplayer_credit_template = value_for_key(
                values, "ScoreLayout.MultiPlayer.CreditIndicator.format", "FREE PLAY"
            )
        ...
        if ball_num_data is None:
            ...
            self.multiplayer_ball_number_template = (
                self.single_player_ball_number_template
            )
        else:
            self.multiplayer["ball_number"] = (
                self.game.dmdHelper.generateTextLayerFromYaml(ball_num_data)
            )
            self.multiplayer_ball_number_template = value_for_key(
                values, "ScoreLayout.MultiPlayer.BallNumber.format", "BALL _"
            )
            if "_" in self.multiplayer_ball_number_template:
                self.multiplayer["ball_number"].set_text(
                    self.multiplayer_ball_number_template.replace(
                        "_", "%d" % self.game.ball
                    )
                )
            else:
        ...
        self.multiplayer["score"] = self.game.dmdHelper.generateTextLayerFromYaml(
            value_for_key(values, "ScoreLayout.MultiPlayer.ActivePlayer")
        )
        self.multiplayer_layers.append(self.multiplayer["score"])

        for idx, playerName in enumerate(
            ["PlayerOne", "PlayerTwo", "PlayerThree", "PlayerFour"]
        ):
            # Make the Player# Score Indicator TextLayer
            self.multiplayer["p%d" % idx] = (
                self.game.dmdHelper.generateTextLayerFromYaml(
                    value_for_key(values, "ScoreLayout.MultiPlayer.%s" % playerName)
                )
            )
    ...
    def update_layer(self):
        ...
        updates_needed = (
            ...
            or (self.game.current_player() is not None)
            and (
