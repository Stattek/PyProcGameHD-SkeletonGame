...
class Replay(Mode):
    ...
    def mode_started(self):
        ...
        self.replay_percentage = float(
            self.game.user_settings["Replay"]["Replay Percentage"]
        )
        self.replay_boost = self.game.user_settings["Replay"]["Replay Boost"]
        self.default_scores = [
            self.game.user_settings["Replay"]["Replay Level 1"],
    ...
    def calc_auto_replay_score(self):
        return (
            int(
                int(self.game.game_data["Audits"]["Avg Score"])
                * ((100.0 - self.replay_percentage) / 100)
            )
            / 10000
        ) * 10000
