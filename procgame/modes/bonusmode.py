  ...
  """
    ...
    def getBonusFrame(self, bonusName, bonusCount, bonusValue):
        ...
        bottomText = (
            str(bonusValue)
            if bonusCount == 1
            else "%dx @ %d" % (bonusCount, bonusValue)
        )
        lyrBottom.set_text(bottomText, style=fs)
        gl = GroupedLayer(
            self.game.dmd.width, self.game.dmd.height, layers=[lyrBottom, lyrTop]
        )
        gl.opaque = True
        return gl
    ...
    def next_bonus_display(self):
        ...
        if len(self.bonus_list) > 0:
            ...
            self.layer = self.getBonusFrame(
                this_bonus["name"], this_bonus["count"], this_bonus["points"]
            )
            self.game.score(this_bonus["count"] * this_bonus["points"])
            self.delay(
                name="next_bonus_display", delay=2, handler=self.next_bonus_display
            )
        else:
            self.layer = None
