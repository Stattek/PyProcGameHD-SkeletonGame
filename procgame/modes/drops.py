...
class BasicDropTargetBank(Mode):
    ...
    def __init__(self, game, priority, prefix, letters):
        ...
        for name in self.names():
            self.add_switch_handler(
                name=name, event_type="open", delay=None, handler=self.dropped
            )
    ...
    def dropped(self, sw):
        ...
        if not self.paused and self.state[sw.name] == "up":
            self.game.lamps[sw.name].schedule(
                schedule=0xF0F0F0F0, cycle_seconds=1, now=True
            )
    ...
    def chase_lamps(self):
        ...
        for name in self.names():
            self.game.lamps[name].schedule(
                schedule=(schedule | (schedule << 16)), cycle_seconds=4, now=True
            )
            schedule <<= bits
    ...
    def schedule_delayed_reset(self, delay):
        ...
        self.delay(
            ...
            event_type=None,
            delay=delay,
            handler=self.reset_drop_target_bank,
        )
...
class ProgressiveDropTargetBank(BasicDropTargetBank):
    ...
    def __init__(self, game, priority, prefix, letters, advance_switch):
        ...
        self.add_switch_handler(
            ...
            event_type="closed",
            delay=None,
            handler=self.__advance_triggered,
        )
    ...
    def advance(self):
        ...
        if new_target == None:
            ...
            self.game.lamps[self.current_target].schedule(
                schedule=0xF0F0F0F0, cycle_seconds=0, now=True
            )
