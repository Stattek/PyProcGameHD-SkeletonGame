...
class BallSearch(Mode):
    ...
    def __init__(
        ...
        for switch in reset_switches:
            self.add_switch_handler(
                name=str(switch),
                event_type=str(reset_switches[switch]),
                delay=None,
                handler=self.reset,
            )
        ...
        for switch in stop_switches:
            self.add_switch_handler(
                name=str(switch),
                event_type=str(stop_switches[switch]),
                delay=None,
                handler=self.stop,
            )
        ...
        if len(reset_switches) == 0 and len(stop_switches) == 0:
            self.logger.warning(
                "Initialized without any stop/reset switches.  Removing functionality"
            )
    ...
    def __perform_search(
        self, completion_wait_time, completion_handler=None, silent=False
    ):
        pass
    ...
    def reset(self, sw):
        if self.enabled:
            ...
            if schedule_search:
                ...
                self.delay(
                    name="ball_search_countdown",
                    event_type=None,
                    delay=self.countdown_time,
                    handler=self.perform_search,
                    param=0,
                )
                self.logger.debug(
                    "RESET via '%s'; will search in %ds." % (sw, self.countdown_time)
                )
            else:
                self.logger.debug(
                    "RESET (requested); next search is not scheduled due to current switch states."
                )
    ...
    def perform_search(
        ...
        self.logger.debug(
            "perform_search(completion_wait_time=%d)" % completion_wait_time
        )

        if completion_wait_time != 0:
            self.logger.info("Initiated")
            if not silent:
                self.game.notifyModes(
                    "evt_balls_missing", args=None, event_complete_fn=None
                )
        self.completion_handler = completion_handler
        delay = 0.150
        for coil in self.coils:
            self.delay(
                name="ball_search_coil1",
                event_type=None,
                delay=delay,
                handler=self.pop_coil,
                param=str(coil),
            )
            delay = delay + 0.150
        self.delay(
            name="start_special_handler_modes",
            event_type=None,
            delay=delay + completion_wait_time,
            handler=self.start_special_handler_modes,
        )
    ...
    def start_special_handler_modes(self):
        ...
        for special_handler_mode in self.special_handler_modes:
            if special_handler_mode not in self.game.modes:
                ...
                self.delay(
                    name="remove_special_handler_mode",
                    event_type=None,
                    delay=d,
                    handler=self.remove_special_handler_mode,
                    param=special_handler_mode,
                )
                d += 3
