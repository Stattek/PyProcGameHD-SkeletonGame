...
class FakePinPROC:
    ...
    def get_events(self):
        ...
        missed_dmd_events = min(
            int(seconds_since_last_dmd_event * float(self.frames_per_second)), 16
        )
    ...
    # Switch rule methods
    def switch_update_rule(
        self, num, state, rule_params, drivers, drive_outputs_now=False
    ):
        ...
        if state == "closed_debounced":
            ...
            pr_state = pinproc.EventTypeSwitchOpenNondebounced
...
class FakePinPROCPlayback(FakePinPROC):
    ...
    _start_time = (
        0  # The simulator start time so we know how to calculate simulator time
    )
    ...
    def __init__(self, machine_type):
        ...
        self._playback_file = open(
            "playback.txt", "r"
        )  # Open our playback file for reading
        ...
        self._event_timestamps = (
            self._events.keys()
        )  # Populate our list of timestamps from the events dictionary keys
        self._event_timestamps.sort()  # Sort timestamps from least to greatest so we access all events in order

        self._start_time = (
            time.clock() * 1000
        )  # Mark down the current start time so we know when to process an event
    ...
    def get_events(self):
        ...
        while (
            len(self._event_timestamps) > 0
            and self._event_timestamps[0] <= current_time
        ):
            evt = self._events[self._event_timestamps[0]]
            print(
                "[%s] [%s] Firing switch %s"
                % (str(current_time), str(self._event_timestamps[0]), evt["swname"])
            )
