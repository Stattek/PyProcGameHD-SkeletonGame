...
class DictWithDefault(dict):
    ...
    def __missing__(self, key):
        logging.error(
            "***ASSET ERROR: An asset with key '%s' has not been loaded.  Using default '%s' instead"
            % (key, self.miss_key)
        )
        if not self.miss_key in self:
            raise KeyError(
                "***ASSET ERROR: An asset with key '%s' has not been loaded.  And default '%s' could not be found to be used instead!"
                % (key, self.miss_key)
            )
...
class AssetManager(object):
    ...
    def loadConfig(self, curr_file_path, filename=None):
        ...
        try:
            ...
            self.logger.error(
                "Error loading asset config file from %s; your configuration file has a syntax error in it!\nDetails: %s",
                path,
                e,
            )
    ...
    def __init__(self, game, yaml_values=None, yaml_file=None):
        ...
        self.rect_color = self.value_for_key_path(
            "UserInterface.progress_bar.border", (120, 120, 120, 255)
        )
        self.inner_rect_color = self.value_for_key_path(
            "UserInterface.progress_bar.fill", (255, 84, 84, 255)
        )
        self.bar_x = self.value_for_key_path("UserInterface.progress_bar.x_center", 0.5)
        self.bar_y = self.value_for_key_path(
            "UserInterface.progress_bar.y_center", 0.25
        )
        ...
        self.text_color = self.value_for_key_path(
            "UserInterface.text.color", (255, 255, 0, 255)
        )
        ...
        self.prog_bar_x = int(
            (self.screen_width * self.bar_x) - (self.prog_bar_width / 2)
        )
        self.prog_bar_y = int(
            (self.screen_height * self.bar_y) - (self.prog_bar_height / 2)
        )
    ...
    def updateProgressBar(self, displayType, fname):
        if self.splash_image is not None:
            sdl2_DisplayManager.inst().screen_blit(
                self.splash_image, expand_to_fill=True
            )
        else:
        ...
        sdl2_DisplayManager.inst().draw_rect(
            ...
            (
                ...
                self.prog_bar_y,
                self.prog_bar_width,
                self.prog_bar_height,
            ),
            False,
        )
        percent = int(
            float(self.numLoaded + 1) / float(self.total) * self.prog_bar_width
        )
        ...
        sdl2_DisplayManager.inst().draw_rect(
            ...
            (
                ...
                self.prog_bar_y + 2,
                percent,
                self.prog_bar_height - 4,
            ),
            True,
        )

        if self.single_line:
            s = "Loading %s: [%06d] of [%06d]: %s" % (
                displayType,
                self.numLoaded + 1,
                self.total,
                fname,
            )
            tx = sdl2_DisplayManager.inst().font_render_text(
                s,
                font_alias=None,
                size=None,
                width=300,
            ...
            sdl2_DisplayManager.inst().screen_blit(
                tx, x=60, y=self.text_y, expand_to_fill=False
            )
        else:
            s = "Loading %s: [%06d] of [%06d]:" % (
                displayType,
                self.numLoaded + 1,
                self.total,
            )
            ...
            sdl2_DisplayManager.inst().screen_blit(
                tx, x=80, y=self.text_y + 35, expand_to_fill=False
            )
    ...
    def loadIntoCache(
        ...
        if self.loaded_map.has_key(file) and not streaming_load:
            ...
            if not streaming_load:
                tmp = dmd.Animation().load(
                    self.dmd_path + file, composite_op=composite_op
                )
        ...
        if streaming_load:
            self.animations[key] = dmd.MovieLayer(
                opaque,
                hold=holdLastFrame,
                repeat=repeatAnim,
                frame_time=frametime,
                movie_file_path=self.dmd_path + file,
                transparency_op=composite_op,
            )
        else:
            self.animations[key] = dmd.AnimatedLayer(
                frames=tmp.frames,
                frame_time=frametime,
                repeat=repeatAnim,
                hold=holdLastFrame,
    ...
    def load(self):
        ...
        for path in paths:
            ...
            lamps += self.value_for_key_path(keypath="LampShows", default={}) or list()
            rgbshows += (
                self.value_for_key_path(keypath="RGBShows", default={}) or list()
            )
            sounds = self.value_for_key_path(keypath="Audio", default={}) or list()
            music += value_for_key(sounds, "Music", {}) or list()
            effects += value_for_key(sounds, "Effects", {}) or list()
        ...
        try:
            ...
            for l in lamps:
                ...
                for tr in self.game.lampctrl.show.lampshow.tracks:
                    ...
                    if tr.driver == None:
                        raise ValueError(
                            "Name '%s' specified in lampshow does not match a driver in the machine yaml."
                            % tr.name
                        )
            ...
            for f in hfonts:
                ...
                if file_path is not None:
                    ...
                    if not os.path.isfile(file_path):
                        raise ValueError(
                            "Could not load font as specified in yaml\n %s\n File [%s] does not exist."
                            % (current, file_path)
                        )
        ...
        for s in effects:
            ...
            self.game.sound.register_sound(
                k, self.game.sfx_path + fname, volume=volume, is_voice=is_voice
            )
            self.numLoaded += 1
        ...
        for s in voice:
            ...
            self.game.sound.register_sound(
                k, self.game.voice_path + fname, volume=volume, is_voice=True
            )
            self.numLoaded += 1
