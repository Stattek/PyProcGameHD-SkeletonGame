...
try:
    ...
    logging.getLogger("game.sound").error(
        "Error importing pygame.mixer; sound will be disabled!  Error: " + str(e)
    )
...
DUCKED_MUSIC_PERCENT = (
    0.5  # drops the music volume to this percentage when ducking for voice lines
)
...
class SoundController(
    mode.Mode
):  # made this a mode since I want to use delay feature for audio queuing
    ...
    def __init__(self, game, priority=10):
        ...
        try:
            mixer.pre_init(
                frequency=22050, size=-16, channels=2, buffer=256
            )  # 256 prev
            mixer.init()
        ...
        self.__set_volume(
            0.5
        )  # service mode should set the volume when it loads the last settings
        self.volume_list = [
            0.00,
            0.05,
            0.10,
    ...
    def play_music(self, key, loops=0, start_time=0.0, channel=CH_MUSIC):
        ...
        if key in self.music:
            ...
            if channel > 0:
                ...
                if start_time > 0:
                    raise ValueError(
                        "play_music does not support start_time>0 for non standard channels"
                    )

                vol = self.music[key][0]["volume"]
                sound_file = self.music[key][0]["file"]
                mixer.Channel(channel).set_volume(
                    self.volume * self.music_ducking_effect
                )
    ...
    def register_sound(
        self, key, sound_file, channel=CH_SFX, volume=0.4, is_voice=False
    ):
        """ """
        ...
        if os.path.isfile(sound_file):
            ...
            if key in self.sounds:
                if not new_sound in self.sounds[key]["sound_list"]:
                    ...
                    self.sounds[key]["is_voice"] = (
                        self.sounds[key]["is_voice"] or is_voice
                    )
            else:
                self.sounds[key] = {"is_voice": is_voice, "sound_list": [new_sound]}
        else:
            self.logger.error(
                "Sound registration error: file %s does not exist!", sound_file
            )

    def register_music(self, key, music_file, volume=0.4, streaming_load=True):
        """ """
        if not self.enabled:
            return
        if os.path.isfile(music_file):
            ...
            if key in self.music:
                if not music_file in self.music[key]:
                    self.music[key].append(
                        {"file": music_file, "volume": volume, "sound_obj": sound_obj}
                    )
            else:
                self.music[key] = [
                    {"file": music_file, "volume": volume, "sound_obj": sound_obj}
            ...
            self.logger.error(
                "Music registration error: file %s does not exist!", music_file
            )
    ...
    def check_voice_finished(self):
        ...
        self.logger.info(
            "cvf: checking if voice playback is complete (pyg queue: %s)" % queued_sound
        )
        ...
        if queued_sound:
            ...
            self.logger.info(
                "cvf: Found an element in the pygame audio queue - check again in %f"
                % delay_length
            )
            self.delay(
                delay=delay_length,
                handler=self.check_voice_finished,
                name="voice_finished",
            )
            return
        ...
        if len(self.queue) > 0:
            ...
            self.delay(
                delay=length, handler=self.check_voice_finished, name="voice_finished"
            )
            self.logger.info(
                "cvf: Feeding an element from self.q into pygame audio queue (len(q)=%d)- check again in %f"
        ...
        if mixer.Channel(CH_VOICE).get_busy():
            self.logger.info(
                "cvf: No queued elements but sound still playing - stalling"
            )
            self.delay(
                delay=0.1, handler=self.check_voice_finished, name="voice_finished"
            )
        else:
            self.logger.info(
                "cvf: voice playback complete.  Won't check until next play_voice()"
            )
    ...
    def play(self, key, loops=0, max_time=0, fade_ms=0, channel=None):
        ...
        if key in self.sounds:
            ...
            if channel is not None and channel.__class__.__name__ == "Channel":
                ...
                channel.play(
                    self.sounds[key]["sound_list"][0], loops, max_time, fade_ms
                )
                return channel
            elif (channel == CH_VOICE) or (
                self.sounds[key]["is_voice"] == True and channel == None
            ):
    ...
    def play_voice(self, key, action=PLAY_QUEUED, tag=None):
        ...
        if action == PLAY_NOTBUSY:
            if mixer.Channel(CH_VOICE).get_busy() or (
                mixer.Channel(CH_VOICE).get_queue() is not None
            ):
                self.logger.debug(
                    "play_voice(key=%s, action=%s) - Voice module already busy - returning"
                    % (key, action)
                )
                return 0
        ...
        if key in self.sounds:
            if (
                action == PLAY_FORCE
            ):  # we need to clear our queue and stop anything playing
                # dump our queue
                self.logger.debug(
                    "play_voice(key=%s, action=%s) - FORCE requested, flushing queue"
                    % (key, action)
                )
                self.queue.clear()
                ...
                mixer.Channel(
                    CH_VOICE
                ).stop()  # incase there was something in the pygame queue?
                # cancel sound completed callback, since it's no longer relevant
                self.cancel_delayed(name="voice_finished")

            # Check if there is a queue
            if (
                mixer.Channel(CH_VOICE).get_queue() or len(self.queue) > 0
            ):  # PLAY_FORCE will have cleared the queue:
                ...
                length = 1
                self.logger.debug(
                    "play_voice(key=%s, action=%s) - queued to wait (len(q)=%d))"
                    % (key, action, len(self.queue))
                )
            else:
                ...
                if action == PLAY_QUEUED:
                    ...
                    if self.is_delayed("voice_finished"):
                        self.logger.debug(
                            "play_voice(key=%s, action=%s) - will play next (queue extended by %f)"
                            % (key, action, length)
                        )
                        self.extend_delay_by("voice_finished", length)
                    else:
                        self.logger.debug(
                            "play_voice(key=%s, action=%s) - will play next (new queue check in %f)"
                            % (key, action, length)
                        )
                        self.delay(
                            delay=length,
                    ...
                    self.logger.debug(
                        "play_voice(key=%s, action=%s) - playing RIGHT NOW (next queue check in %f)"
                        % (key, action, length)
                    )
    ...
    def volume_up(self):
        ...
        try:
            ...
            volume_idx = int(
                self.get_max_volume() / 2
            )  # resets the volume to 1/2 volume
    ...
    def __set_volume(self, new_volume):
        ...
        mixer.music.set_volume(
            new_volume * self.music_ducking_effect * self.current_music_track_volume
        )
        mixer.Channel(CH_MUSIC_1).set_volume(
            new_volume * self.music_ducking_effect
        )  # for these, the sound file has
        mixer.Channel(CH_MUSIC_2).set_volume(
            new_volume * self.music_ducking_effect
        )  # self.current_music_track_volume
