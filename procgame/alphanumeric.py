    ...
    def display(self, input_strings, intensities=[[1] * 16] * 2):
        ...
        for i in range(16):
            ...
            for j in range(2):
                ...
                commands += [
                    pinproc.aux_command_output_custom(
                        segs[j][i] & 0xFF, 0, self.strobes[j * 2 + 1], False, 0
                    )
                ]
                commands += [
                    pinproc.aux_command_output_custom(
                        (segs[j][i] >> 8) & 0xFF, 0, self.strobes[j * 2 + 2], False, 0
                    )
                ]
                char_on_time += [intensities[j][i] * self.full_intensity_delay]
                char_off_time += [
                    self.inter_char_delay
                    + (self.full_intensity_delay - char_on_time[j])
                ]
            ...
            commands += [
                pinproc.aux_command_output_custom(
                    0, 0, self.strobes[first * 2 + 1], False, 0
                )
            ]
            commands += [
                pinproc.aux_command_output_custom(
                    0, 0, self.strobes[first * 2 + 2], False, 0
                )
            ]
            ...
            commands += [
                pinproc.aux_command_output_custom(
                    0, 0, self.strobes[second * 2 + 1], False, 0
                )
            ]
            commands += [
                pinproc.aux_command_output_custom(
                    0, 0, self.strobes[second * 2 + 2], False, 0
                )
            ]
