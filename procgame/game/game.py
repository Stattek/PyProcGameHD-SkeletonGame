...
def config_named(name):
    if not os.path.isfile(
        name
    ):  # If we cannot find this file easily, try searching the config_path:
...
class GameController(object):
    ...
    def load_config(self, filename):
        ...
        if not self.config:
            raise ValueError(
                'load_config(filename="%s") could not be found. Did you set config_path?'
                % (filename)
            )
        self.process_config()
    ...
    def load_config_stream(self, stream):
        ...
        if not self.config:
            raise ValueError(
                "load_config_stream() could not load configuration.  Malformed YAML?"
            )
        self.process_config()
        ...
        for section, collection, klass in pairs:
            if section in self.config:
                ...
                for name in sect_dict:
                    ...
                    if self.machine_type == pinproc.MachineTypePDB:
                        number = pdb_config.get_proc_number(
                            section, str(item_dict["number"])
                        )
                        if number == -1:
                            self.logger.error(
                                "%s Item: %s cannot be controlled by the P-ROC.  Ignoring...",
                                section,
                                name,
                            )
                            continue
                    else:
                        number = pinproc.decode(
                            self.machine_type, str(item_dict["number"])
                        )

                    item = None
                    if (
                        "bus" in item_dict and item_dict["bus"] == "AuxPort"
                        ...
                        if klass == Switch:
                            if (
                                "debounce" in item_dict
                                and item_dict["debounce"] == False
        ...
        for virtual_driver in new_virtual_drivers:
            ...
            for collection in [self.coils, self.lamps]:
                ...
                for item in items_to_remove:
                    ...
                    collection.add(
                        item[name],
                        VirtualDriver(self, item[name], item[number], polarity),
                    )
        ...
        *user_filename* contains the values set by the user.
        ...
        for section in self.settings:
            for item in self.settings[section]:
                ...
                if not item in self.user_settings[section]:
                    settings_changed = True
                    self.logger.error(
                        "Missing setting in user settings file; will be replaced with default:\n%s:{%s}\n-------"
                        % (section, item)
                    )
                    if "default" in self.settings[section][item]:
                        self.user_settings[section][item] = self.settings[section][
                            item
                        ]["default"]
                    else:
                        self.user_settings[section][item] = self.settings[section][
                    ...
                    if "increments" not in self.settings[section][item]:
                        if (
                            self.user_settings[section][item]
                            not in self.settings[section][item]["options"]
                        ):
                            settings_changed = True
                            self.logger.error(
                                "Invalid value found in user settings file; will be replaced with default:\n%s:{%s}\n-------"
                                % (section, item)
                            )
                            if "default" in self.settings[section][item]:
                                self.user_settings[section][item] = self.settings[
                                    section
                                ][item]["default"]
                            else:
                                self.user_settings[section][item] = self.settings[
        ...
        for section in self.user_settings:
            if section not in self.settings:
                settings_changed = True
                self.logger.error(
                    "Deprecated section found in user settings file; will be removed:\n%s\n-------"
                    % section
                )
                ...
                for item in self.user_settings[section]:
                    if item not in self.settings[section]:
                        settings_changed = True
                        self.logger.error(
                            "Deprecated setting found in user settings file; will be removed:\n%s:{%s}\n-------"
                            % (section, item)
                        )
    ...
    def save_settings(self, filename):
        ...
        if os.path.getsize(filename) == 0:
            self.logger.error(
                " ****** CORRUPT GAME USER SETTINGS FILE REPLACING WITH CLEAN DATA  --- restoring last copy ****************"
            )
            # remove bad file
            os.remove(filename)
    ...
    def save_game_data(self, filename):
        ...
        if os.path.getsize(filename) == 0:
            self.logger.info(
                " ****************   CORRUPT DATA FILE REPLACING WITH CLEAN DATA  --- restoring last copy ****************"
            )
            # remove bad file
            os.remove(filename)
            os.rename(filename + ".bak", filename)
    ...
    def enable_flippers(self, enable):
        ...
        for flipper in self.config["PRFlippers"]:
            ...
            if self.coils.has_key(flipper + "Hold"):
                style = "wpc"
                self.logger.info(
                    "%sabling WPC style flipper" % ("En" if enable else "Dis")
                )
                hold_coil = self.coils[flipper + "Hold"]
            else:
                self.logger.info(
                    "%sabling Stern style flipper" % ("En" if enable else "Dis")
                )
                style = "stern"
            ...
            if enable:
                if style == "wpc":
                    drivers += [
                        pinproc.driver_state_pulse(
                            main_coil.state(), main_coil.default_pulse_time
                        )
                    ]
                    ...
                    drivers += [
                        pinproc.driver_state_patter(
                            main_coil.state(), 2, 18, main_coil.default_pulse_time, True
                        )
                    ]
            self.proc.switch_update_rule(
                switch_num,
    ...
    def enable_bumpers(self, enable):
        ...
        for bumper in self.config["PRBumpers"]:
            ...
            if enable:
                drivers += [
                    pinproc.driver_state_pulse(coil.state(), coil.default_pulse_time)
                ]
    ...
    def install_switch_rule_coil_disable(
        self,
        switch_num,
        switch_state,
        coil_name,
        ...
        self.proc.switch_update_rule(
            switch_num,
            switch_state,
            {"notifyHost": notify_host, "reloadActive": reload_active},
            drivers,
            drive_coil_now_if_valid,
        )

    def install_switch_rule_coil_pulse(
        self,
        switch_num,
        switch_state,
        coil_name,
    ...
    def install_switch_rule_coil_schedule(
        self,
        switch_num,
        switch_state,
        coil_name,
        ...
        if enable:
            drivers += [
                pinproc.driver_state_schedule(
                    coil.state(), schedule, schedule_seconds, now
                )
            ]
        self.proc.switch_update_rule(
            switch_num,
    ...
    def install_switch_rule_coil_patter(
        self,
        switch_num,
        switch_state,
        coil_name,
    ...
    def process_event(self, event):
        ...
        if event_type == 99:  # CTRL-C to quit
            ...
            event_type == pinproc.EventTypeBurstSwitchOpen
            or event_type == pinproc.EventTypeBurstSwitchClosed
        ):
            self.burst_event(event)
        elif (
            event_type == pinproc.EventTypeSwitchClosedDebounced
            or event_type == pinproc.EventTypeSwitchOpenDebounced
            or event_type == pinproc.EventTypeSwitchClosedNondebounced
            or event_type == pinproc.EventTypeSwitchOpenNondebounced
        ):
    ...
    def other_event(self, event):
        self.logger.warning(
            "Unknown event type received.  Type:%d, Value:%s."
    ...
    def switch_event(self, event):
        ...
        try:
            ...
            self.logger.warning(
                "Received switch event but couldn't find switch %s." % event_value
            )
            return
    ...
    def get_events(self):
        ...
        events.extend(
            self.get_virtualDMDevents()
        )  # MJO: changed to support fake DMD w/o h/w DMD
        return events
    ...
    def get_virtualDMDevents(self):
        ...
        if missed_dmd_events > 0:
            ...
            events.extend(
                [{"type": pinproc.EventTypeDMDFrameDisplayed, "value": 0}]
                * missed_dmd_events
            )
        ...
        try:
            ...
            if loops != 0:
                ...
                if self.dmd_updates > 0:
                    ...
                    self.logger.info(
                        "loops between dmd updates: %0.3f", (loops / self.dmd_updates)
                    )
