...
try:
    ...
    )  # This is used to put commas in the score
except Exception, e:
    # Windows
...
def run_proc_game(game_class):
    ...
    try:
        game = game_class()
        game.run_loop(0.0001)
    except Exception as _:
        # back up the exception traceback
        last_exc_str = traceback.format_exc()
    finally:
        ...
        if last_exc_str is not None:
            # we found an error, just raise another exception with the traceback
            raise RuntimeError(f"Found exception: {last_exc_str}")
...
class SkeletonGame(BasicGame):
    ...
    def __init__(self, machineYamlFile, curr_file_path, machineType=None):
        try:
            ...
            if machineType is None:
                ...
                if not self.config:
                    raise ValueError(
                        'Yaml file "%s" could not be loaded and machineType not passed as arg to SkeletonGame() init.'
                        % (machineYamlFile)
                    )
            ...
            if not machine_type:
                raise ValueError(
                    'machine config(filename="%s") did not set machineType, and not set in SkeletonGame() init.'
                    % (machineYamlFile)
                )
            ...
            self.lampshow_path = config.value_for_key_path(
                "lampshow_path", curr_file_path + "/assets/lampshows/"
            )
            self.dmd_path = config.value_for_key_path(
                "dmd_path", curr_file_path + "/assets/dmd/"
            )
            self.sound_path = config.value_for_key_path(
                "sound_path", curr_file_path + "/assets/sound/"
            )

            self.voice_path = self.sound_path + config.value_for_key_path(
                "voice_dir", "voice/"
            )
            self.sfx_path = self.sound_path + config.value_for_key_path(
                "sfx_dir", "sfx/"
            )
            self.music_path = self.sound_path + config.value_for_key_path(
                "music_dir", "music/"
            )

            self.hdfont_path = config.value_for_key_path(
                "hdfont_dir", curr_file_path + "/assets/fonts/"
            )
            ...
            self.known_events = [
                "evt_tilt",
                "evt_shoot_again",
                "evt_single_ball_play",
                "evt_ball_ending",
                "evt_ball_starting",
                "evt_ball_saved",
                "evt_game_ending",
                "evt_game_starting",
                "evt_tilt_ball_ending",
                "evt_player_added",
                "evt_balls_missing",
                "evt_balls_found",
                "evt_volume_up",
            ...
            self.sg_event_queue = (
                []
            )  # if a known event fires while still processing another, this will
            # hold the queue of out-standing SG events

            self.event = None  # the current SG event being processed

            self.game_tilted = (
                False  # indicates if any kind of tilt has occured; tilt, slam_tilt
            )
            ...
            self.use_stock_scoredisplay = config.value_for_key_path(
                "default_modes.score_display", True
            )
            self.use_stock_bonusmode = config.value_for_key_path(
                "default_modes.bonus_tally", True
            )
            self.use_stock_attractmode = config.value_for_key_path(
                "default_modes.attract", True
            )
            self.use_osc_input = config.value_for_key_path(
                "default_modes.osc_input", True
            )
            self.use_stock_servicemode = config.value_for_key_path(
                "default_modes.service_mode", True
            )
            # self.use_HD_servicemode = config.value_for_key_path('default_modes.service_mode_HD', True)
            self.use_stock_tiltmode = config.value_for_key_path(
                "default_modes.tilt_mode", True
            )
            self.use_ballsearch_mode = config.value_for_key_path(
                "default_modes.ball_search", True
            )
            self.use_multiline_score_entry = config.value_for_key_path(
                "default_modes.multiline_highscore_entry", False
            )
            self.ballsearch_time = config.value_for_key_path(
                "default_modes.ball_search_delay", 30
            )
            ...
            self.service_mode = service.ServiceMode(
                self, 99, self.fonts["settings-font-small"], extra_tests=[]
            )
            ...
            if self.use_stock_tiltmode:
                ...
                self.tilt_mode = TiltMonitorMode(
                    game=self,
                    priority=99998,
                    tilt_sw=tilt_sw_name,
                    slam_tilt_sw=slamtilt_sw_name,
                )

            shoot_again = self.lamps.item_named_or_tagged("shoot_again")
            shooter_lane_sw_name = self.find_item_name("shooter", self.switches)
            self.ball_save = ballsave.BallSave(
                self, lamp=shoot_again, delayed_start_switch=shooter_lane_sw_name
            )
            ...
            if len(trough_switchnames) == 0:
                logging.getLogger("Trough").warning(
                    "No switches have been tagged 'trough'.  Switches with names that start Trough will be used."
                )
                trough_switchnames = [
                    t.name for t in self.switches if t.name.startswith("trough")
                ]
            else:
                trough_switchnames = [t.name for t in trough_switchnames]

            if len(trough_switchnames) == 0:
                logging.getLogger("Trough").error(
                    "No switches have been tagged 'trough' and no switch names start with 'trough'"
                )
                raise ValueError(
                    "Machine YAML must contain switches either tagged 'trough' or with names that start with 'trough'"
                )

            early_save_switchnames = [
                save_sw.name for save_sw in self.switches.items_tagged("early_save")
            ]
            ...
            if "trough" in self.coils:
                ...
                if trough_coil_name is None:
                    raise ValueError(
                        "machine YAML must define a coil named 'Trough' or that starts with"
                    )
            ...
            if shooter_lane_sw_name is None:
                raise ValueError(
                    "machine YAML must define a switch named 'shooter' or tagged 'shooter'"
                )
            self.trough = Trough(
                self,
                trough_switchnames,
                trough_switchnames[-1],
                trough_coil_name,
                early_save_switchnames,
                shooter_lane_sw_name,
                drain_callback=None,
                plunge_coilname=plunge_coilname,
                autoplunge_settle_time=autoplunge_settle_time,
                trough_settle_time=trough_settle_time,
            )
            ...
            bs_stopSwitches = [
                ...
                if (
                    hasattr(sw, "ballsearch")
                    and (sw.ballsearch == "stop" or "stop" in sw.ballsearch)
                )
            ]
            bs_resetSwitches = [
                sw
                ...
                if (
                    hasattr(sw, "ballsearch")
                    and (sw.ballsearch == "reset" or "reset" in sw.ballsearch)
                )
            ]
            ...
            self.ballsearch_coils = [
                dr.name
                for dr in self.coils
                if (hasattr(dr, "ballsearch") and dr.ballsearch == True)
            ]

            if self.use_ballsearch_mode:
                if len(bs_stopSwitches) == 0 and len(bs_resetSwitches) == 0:
                    self.logger.error(
                        "Could not use Ball search mode as there were no ballsearch tags (reset/stop) on any game switches in the yaml.\n -- will default to using your games: do_ball_search() method..."
                    )
                    self.use_ballsearch_mode = False

            # create it anyway; if the switches are empty it will nerf itself.
            self.ball_search = BallSearch(
                self,
                priority=100,
                countdown_time=self.ballsearch_time,
            ...
            if self.use_osc_input:
                try:
                    ...
                    self.osc_closed_switches = []
                self.osc = osc.OSC_Mode(
                    game=self, priority=1, closed_switches=self.osc_closed_switches
                )
    ...
    def end_run_loop(self):
        if (
            not self.cleaned_up
        ):  # if the game hasn't crashed, this might be called twice
            cleanup()
    ...
    def set_status(self, msg, duration=2.0):
        ...
        self.displayText(
            msg,
            font_key=self.status_font_name,
            background_layer="status_bg",
            font_style=self.status_font_style,
            flashing=8,
            duration=duration,
        )
    ...
    def generateLayer(
        self, msg, background_layer=None, font_style=None, font_key=None, opaque=False
    ):
        ...
        return self.dmdHelper.genMsgFrame(
            msg, background_layer, font_style, font_key, opaque
        )

    def ball_saver_enable(
        self,
        num_balls_to_save=1,
        time=5,
        now=True,
    ...
    def enable_ball_saver(
        ...
        self.logger.debug(
            "ball saver enabled balls=[%d], time left=[%d]" % (num_balls_to_save, time)
        )
        self.ball_save.start(
            num_balls_to_save, time, now, allow_multiple_saves, tick_rate
        )
    ...
    def notifyOfNewMode(self, new_mode):
        ...
        if weakref.ref(new_mode) in self.known_modes[new_mode.mode_type]:
            self.logger.warning(
                "Skel: A second instance of an already known mode has been created '%s'"
                % new_mode
            )
            raise ValueError(
                "This cannot happen. Someone notified us of the same mode instance twice!"
            )

        for m in self.known_modes[new_mode.mode_type]:
            if type(m()) == type(new_mode):
                raise ValueError(
                    "Your code attempted to create a second instance of the Mode '%s' --this is likely an error."
                    % type(new_mode)
                )
    ...
    def add_evt_handler(self, mode, evt_name):
        if evt_name not in self.event_handlers:
            raise ValueError(
                "Mode: %s defined a function named '%s' which is not known to the Event System"
                % (mode, evt_name)
            )
    ...
    def notifyNextMode(self):
        ...
        if len(self.notify_list) == 0:
            ...
            if self.event_complete_fn is not None:
                self.logger.debug(
                    "Skel: completing event '%s' by calling '%s'"
                    % (self.event, self.event_complete_fn)
                )
        ...
        self.logger.debug(
            "Skel: calling mode '%s' event handler for event '%s'"
            % (next_handler, self.event)
        )
        ...
        if d is not None and (type(d) is int or type(d) is float):
            ...
            if d[1] == True:  # flag to stop event propegation and jump to the event
                self.notify_list = (
                    list()
                )  # zero out the list so the next 'notifyNext' call will just call the final event handler
                self.logger.info(
                    "Skel: Mode '%s' indicates event '%s' is now complete.  Blocking further propegation"
                    % (next_handler, self.event)
                )
            if d[0] > 0:
                self.curr_delayed_by_mode = next_handler
    ...
    def notifyNextModeNow(self, caller_mode):
        if caller_mode == self.curr_delayed_by_mode:
            ...
            self.logger.critical(
                "Skel: notifyNextModeNow called by %s, but this mode is not blocking this event! (%s is)!?"
                % (caller_mode, self.curr_delayed_by_mode)
            )

    def checkOutstandingSGEvents(self):
        if len(self.sg_event_queue) > 0:
            self.logger.info(
                "Processing SG Event Queue: contains %d events"
                % len(self.sg_event_queue)
            )
            evt = self.sg_event_queue.pop()
            self.notifyModes(
                evt.name, evt.args, evt.on_complete_fn, evt.only_active_modes
            )
        else:
            self.logger.info("SG Event Queue -- All Clear")

    def notifyModes(
        self, event, args=None, event_complete_fn=None, only_active_modes=True
    ):
        ...
        if self.event is not None:
            ...
            self.logger.error(
                "Trying to notify modes about new event [%s] while [%s] is still being processed!!"
                % (event, self.event)
            )

            self.sg_event_queue.append(
                SGEvent(event, args, event_complete_fn, only_active_modes)
            )
            return
        ...
        if only_active_modes:
            handlers = [
                h()
                for h in self.event_handlers[self.event]
                if h() is not None and h() in self.modes
            ]
        else:
        ...
        for h in handlers:
            self.logger.debug(
                "Skel: event '%s' queuing handler found in mode [%s]" % (event, h)
            )
    ...
    def quickNotifyModes(
        ...
        if self.event is not None:
            # We are still processing an event
            self.logger.info(
                "NOTE: we are notifying modes about a 'quick' event [%s] while a system event [%s] is still being processed!!"
                % (event, self.event)
            )
        ...
        for h in handlers:
            self.logger.debug(
                "Skel: quick event '%s' handler found in mode [%s]" % (event, h)
            )
        ...
        for h in quick_notify_list:
            self.logger.debug(
                "Skel: quick event '%s' being handled in mode [%s]" % (event, h)
            )
            d = h.handle_game_event(event, params=args)
            if (d is not None) and (d != 0):
                self.logger.error(
                    "modes '%s' has a quick event handler for event [%s] that seems to be requesting additional time.  This is not possible!"
                    % (event, h)
                )
    ...
    def reset(self):
        ...
        if self.trough.num_balls() < self.num_balls_total:
            self.logger.info(
                "Skel: RESET: trough isn't full [%d of %d] -- requesting search"
                % (self.trough.num_balls(), self.num_balls_total)
            )
            if self.use_ballsearch_mode:
    ...
    def start_attract_mode(self):
        ...
        self.modes.add(
            self.attract_mode
        )  # plays the attract mode and kicks off the game
    ...
    def enable_alphanumeric_flippers(self, enable):
        ...
        if flipperEnableCoil is not None:
            ...
            self.logger.warning(
                "No flipperEnable entry in PRCoils section of machine yaml"
            )
    ...
    def load_settings_and_stats(self):
        ...
        self.logger.info(
            "VOLUME SET TO: %d / %d" % (game_volume, self.sound.get_max_volume())
        )
    ...
    def ball_starting(self):
        ...
        self.notifyModes(
            "evt_ball_starting", args=None, event_complete_fn=self.actually_start_ball
        )
    ...
    def shoot_again(self):
        ...
        self.notifyModes(
            "evt_shoot_again",
            args=None,
            event_complete_fn=self.ball_starting,
            only_active_modes=False,
        )
    ...
    def __ball_drained_callback(self):
        ...
        if self.trough.num_balls_in_play == 0:
            ...
            self.game_data["Audits"]["Avg Ball Time"] = self.calc_time_average_string(
                self.game_data["Audits"]["Balls Played"],
            ...
            if self.game_tilted:
                ...
                self.notifyModes(
                    "evt_ball_ending",
                    args=(shoot_again, last_ball),
                    event_complete_fn=self.end_ball,
                )
            ...
            if self.trough.num_balls_to_launch >= 1:
                self.logger.warning(
                    "one ball in play, but more balls are pending launch (supressing evt_single_ball_play)"
                )
            elif self.game_tilted:
                self.logger.info(
                    "one ball in play, but game is tilted (supressing evt_single_ball_play)"
                )
            else:
                self.notifyModes(
                    "evt_single_ball_play", args=None, event_complete_fn=None
                )
        else:
    ...
    def tilted(self):
        ...
        if self.trough.drain_callback == None:
            self.logger.debug(
                "machine tilted before drain logic installed; install it now!"
            )
    ...
    def tilted_ball_end(self):
        ...
        if not self.b_slam_tilted:
            self.notifyModes(
                "evt_tilt_ball_ending", args=None, event_complete_fn=self.end_ball
            )
        else:
            self.notifyModes(
                "evt_tilt_ball_ending",
                args=None,
                event_complete_fn=self.slam_tilt_complete,
            )
    ...
    def game_started(self):
        ...
        if self.game_start_pending and (self.trough.num_balls() < self.num_balls_total):
            self.logger.info(
                "Skel: Game START : PLEASE WAIT!! -- TROUGH STATE is still BLOCKING GAME START!"
            )
        ...
        if self.trough.num_balls() < self.num_balls_total:
            ...
            self.logger.info(
                "Skel: game_started: trough isn't full [%d of %d] -- requesting search"
                % (self.trough.num_balls(), self.num_balls_total)
            )
            if self.use_ballsearch_mode:
                self.ball_search.perform_search(3, completion_handler=self.reset_search)
                ...
                self.logger.debug(
                    "Skel: game_started: Programmer custom ball search initiated"
                )
                self.do_ball_search(silent=False)
                self.ball_search.delay(
                    name="ballsearch_start_delay",
                    event_type=None,
                    delay=3.0,
                    handler=self.reset_search,
                )
            return
        ...
        self.notifyModes(
            "evt_game_starting", args=None, event_complete_fn=self.actually_start_game
        )
    ...
    def end_game(self):
        ...
        for i in range(len(self.players)):
            ...
            self.game_data["Audits"]["Avg Score"] = self.calc_number_average(
                self.game_data["Audits"]["Games Played"],
        ...
        self.notifyModes(
            "evt_game_ending", args=None, event_complete_fn=self.game_ended
        )
    ...
    def game_ended(self):
        ...
        seq_manager = highscore.HD_EntrySequenceManager(
            game=self, priority=2, multiline=self.use_multiline_score_entry
        )
        seq_manager.finished_handler = self.high_score_entry_completed
        seq_manager.logic = highscore.CategoryLogic(
            game=self, categories=self.highscore_categories
        )
    ...
    def __pre_high_score_entry(self, seq_manager, category):
        self.notifyModes(
            "evt_initial_entry", args=category, event_complete_fn=seq_manager.prompt
        )
    ...
    def calc_time_average_string(self, prev_total, prev_x, new_value):
        ...
        avg_game_time = int((int(prev_total) * int(prev_time)) + int(new_value)) / (
            int(prev_total) + 1
        )
    ...
    def start_service_mode(self):
        ...
        self.modes.modes = [
            ...
            if isinstance(m, AdvancedMode) and m.mode_type == AdvancedMode.System
        ]
        for m in self.modes:
            self.modes.remove(m)
...
class BonusRecord:
    ...
    @classmethod
    def define_bonus(
        cls, name, point_value, max_per_ball=float("inf"), max_per_game=float("inf")
    ):
        """defines a point value for a specific named bonus"""
        master = find_in_list(name, cls.master_list)
        if master is not None:
            logging.getLogger("Bonus").warning(
                "trying to redefine existing bonus named ['%s']" % name
            )
        else:
    ...
    def award(self, name, quantity=1, point_value=None):
        ...
        if master is None:
            logging.getLogger("Bonus").warning(
                "trying to award bonus ['%s'] which is not defined in master list"
                % name
            )
            master = {
                "name": name,
        ...
        if (
            prev_award["count"] < master["max_ball"]
            and prev_award["count"] < master["max_game"]
        ):
