...
class Mode(object):
    ...
    __children = (
        None  # child modes, managed with add_child_mode() and remove_child_mode()
    )
    ...
    def __scan_switch_handlers(self):
        ...
        for item in dir(self):
            ...
            if switch_name not in self.game.switches:
                raise ValueError(
                    "Unrecognized switch name %s in handler %s.%s()."
    ...
    def add_switch_handler(self, name, event_type, delay, handler):
        ...
        try:
            ...
            self.game.logger.error(
                "add_switch_handler(): Switch %s unknown. Please check your machine configuration file."
                % (name)
            )
            return

        asw = Mode.AcceptedSwitch(
            name=name, event_type=et, delay=delay, handler=handler, param=sw
        )
        if asw not in self.__accepted_switches:
            self.__accepted_switches.append(asw)
        else:
            self.game.logger.error(
                "framework suppressed redundant switch handler: [name=%s, event_type=%s, delay=%s, handler=%s, param=%s]"
                % (name, et, delay, handler, sw)
            )
    ...
    def handle_event(self, event):
        ...
        self.__delayed = filter(
            lambda x: not (sw_name == x.name and x.event_type != event["type"]),
            self.__delayed,
        )

        filt = lambda accepted: (accepted.event_type == event["type"]) and (
            accepted.name == sw_name
        )
    ...
    class AcceptedSwitch:
        ...
        # TODO: this function can be much simpler
        ...
        def __str__(self):
            return "<name=%s event_type=%s delay=%s>" % (
                self.name,
                self.event_type,
                self.delay,
            )
    ...
    class Delayed:
        ...
        def __str__(self):
            return "<name=%s time=%s event_type=%s>" % (
                self.name,
                self.time,
                self.event_type,
            )
...
class ModeQueue(object):
    ...
    def add(self, mode):
        if mode in self.modes:
            raise ValueError(
                "Attempted to add mode " + str(mode) + ", already in mode queue."
            )
        self.modes += [mode]
    ...
    def handle_event(self, event):
        modes = copy.copy(
            self.modes
        )  # Make a copy so if a mode is added we don't get into a loop.
        for mode in modes:
            ...
            if layer:
                log_rows.append(
                    [
                        ...
                        type(mode).__name__,
                        type(layer).__name__,
                        "opaque=%s" % str(layer.opaque),
                        "enabled=%s" % str(layer.enabled),
                    ]
                )
            else:
