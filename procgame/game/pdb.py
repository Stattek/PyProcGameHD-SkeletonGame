...
class LED(GameItem):
    ...
    def __init__(self, game, name, number):
        ...
        self.logger.debug(
            "Creating LED: %s, board_addr: %s, color_addrs: %s",
            self.name,
            self.board_addr,
            self.addrs,
        )
    ...
    def color(self, color):
        ...
        if len(color) >= len(self.addrs):
            ...
            for i in range(len(self.addrs)):
                self.game.proc.led_color(
                    self.board_addr,
                    self.addrs[i],
                    self.normalize_color(
                        int(color[i] * self.brightness_compensation[i])
                    ),
                )
    ...
    def schedule(self, schedule, cycle_seconds=0, now=True, use_color=None):
        ...
        self.logger.warning(
            "Attempted to call schedule(%04x) on PD-LED %s" % (schedule, self.name)
        )
        ...
        if use_color is None:
            if hasattr(self, "default_color"):
                ...
                self.logger.warning(
                    "LED [%s] does not have a default color.  Using white" % self.name
                )
    ...
    def disable(self):
        ...
        for i in range(len(self.addrs)):
            self.game.proc.led_color(
                self.board_addr, self.addrs[i], self.normalize_color(0)
            )
    ...
    def enable(self):
        ...
        for i in range(len(self.addrs)):
            self.game.proc.led_color(
                self.board_addr,
                self.addrs[i],
                self.normalize_color(int(255 * self.brightness_compensation[i])),
            )
...
class PDBConfig(object):
    ...
    def __init__(self, proc, config):
        ...
        for name in config["PRLamps"]:
            ...
            if lamp.lamp_type == "dedicated":
                ...
                lamp_dict = {
                    "source_index": lamp_source_bank_list.index(lamp.source_bank()),
                ...
                lamp_dict_for_index = {
                    "source_board": lamp.source_board(),
                    "sink_bank": lamp.sink_bank(),
                    "source_output": lamp.source_output(),
                }
        ...
        for group_ctr in range(0, 4):
            ...
            self.logger.info(
                "Driver group %02d (dedicated): Enable=%s", group_ctr, enable
            )
            proc.driver_update_group_config(
                group_ctr, 0, group_ctr, 0, 0, False, True, enable, True
            )
        ...
        for coil_bank in coil_bank_list:
            ...
            if group_ctr >= num_proc_banks or coil_bank >= 16:
                ...
                self.logger.info(
                    "Driver group %02d: slow_time=%d Enable Index=%d",
                    group_ctr,
                    0,
                    coil_bank,
                )
                ...
                proc.driver_update_group_config(
                    group_ctr, 0, coil_bank, 0, 0, False, True, True, True
                )
                group_ctr += 1
        ...
        for i in range(group_ctr, 26):
            ...
            proc.driver_update_group_config(
                i, self.lamp_matrix_strobe_time, 0, 0, 0, False, True, False, True
            )
    ...
    def get_globals(self, config):
        if (
            "PRDriverGlobals" in config
            and "lamp_matrix_strobe_time" in config["PRDriverGlobals"]
        ):
            self.lamp_matrix_strobe_time = int(
                config["PRDriverGlobals"]["lamp_matrix_strobe_time"]
            )
        else:
