...
class AttrCollection:
    ...
    def __getattr__(self, attr):
        try:
            ...
            if attr is None:
                raise KeyError(
                    "Something has attempted to reference an item in the collection '%s' using a key with value 'None'"
                    % self.name
                )
            raise KeyError(
                "The collection '%s' does not define an element named/numbered '%s' (exception=%s)"
                % (self.name, attr, e)
            )
    ...
    def item_named_or_tagged(self, identifier):
        ...
        if len(l) == 0:
            ...
            logging.getLogger("SG").warning(
                "Multiple items are tagged '%s' -- only the first will be used." % tag
            )
        return l[0]
...
class Driver(GameItem):
    ...
    def set_color(self, arg, time=0):
        if self.name in self.game.lamps:
            if arg[0] > 128 or arg[1] > 128 or arg[2] > 128:
                self.logger.debug(
                    "set_color enable %s %s for NON wsRGB" % (self.name, str(arg))
                )
                self.enable()
            else:
    ...
    def future_pulse(self, milliseconds=None, timestamp=0):
        ...
        if not milliseconds in range(256):
            raise ValueError("milliseconds must be in range 0-255.")
        self.logger.debug(
            "Driver %s - future pulse %d", self.name, milliseconds, timestamp
        )
    ...
    def patter(self, on_time=10, off_time=10, original_on_time=0, now=True):
        ...
        self.logger.debug(
            "Driver %s - patter on:%d, off:%d, orig_on:%d, now:%s",
            self.name,
            on_time,
            off_time,
            original_on_time,
        ...
        self.game.proc.driver_patter(
            self.number, on_time, off_time, original_on_time, now
        )
        self.last_time_changed = time.time()
    ...
    def pulsed_patter(self, on_time=10, off_time=10, run_time=0, now=True):
        ...
        self.logger.debug(
            "Driver %s - pulsed patter on:%d, off:%d, run_time:%d, now:%s",
            self.name,
            on_time,
            off_time,
            run_time,
        ...
        self.game.proc.driver_pulsed_patter(
            self.number, on_time, off_time, run_time, now
        )
        self.last_time_changed = time.time()
    ...
    def schedule(self, schedule, cycle_seconds=0, now=True):
        ...
        self.game.proc.driver_schedule(
            number=self.number, schedule=schedule, cycle_seconds=cycle_seconds, now=now
        )
        self.last_time_changed = time.time()
...
class VirtualDriver(Driver):
    ...
    def pulse(self, milliseconds=None):
        ...
        self.logger.debug(
            "Time: %f: VirtualDriver %s - pulse %d. End time: %f",
            time.time(),
            self.name,
            milliseconds,
            self.time_ms,
    ...
    def change_state(self, new_state):
        ...
        self.logger.debug(
            "VirtualDriver %s - state change: %d", self.name, self.curr_state
        )
    ...
    def inc_schedule(self):
        self.next_action_time_ms += 0.0325
