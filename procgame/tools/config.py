...
def tool_populate_options(parser):
    parser.add_option(
        "-k", "--key", action="store", help="The configuration key to be manipulated."
    )
    parser.add_option(
        "-a", "--add", action="store", help="Add VALUE to list KEY.", metavar="VALUE"
    )
    parser.add_option(
        "-r",
        "--remove",
        action="store",
        help="Remove VALUE from list KEY.",
        metavar="VALUE",
    )
    parser.add_option(
        "-s",
        "--set",
        action="store",
        help="Assign the value of KEY to VALUE.",
        metavar="VALUE",
    )
    parser.add_option(
        "-c", "--clear", action="store_true", help="Removes KEY from the configuration."
    )
...
def tool_run(options, args):
    ...
    if options.key:
        if options.key not in procgame.config.values and not (
            options.add or options.set
        ):
        ...
        if options.add or options.remove:
            if (
                options.key in procgame.config.values
                and type(procgame.config.values[options.key]) != list
            ):
                print(
                    "Cannot add or remove values from a key that does not have a list value.  Type is "
                    + (type(procgame.config.values[options.key]).__name__)
                )
                sys.exit(3)
        ...
        if options.set:
            if (
                options.key in procgame.config.values
                and type(procgame.config.values[options.key]) != str
            ):
                print(
                    "Cannot assign a value to a key that is not a string.  Type is "
                    + (type(procgame.config.values[options.key]).__name__)
                )
                sys.exit(3)
    ...
    if not os.path.exists(procgame.config.path):
        ...
        print(
            "Your configuration file contains one or more errors and was not parsed successfully."
        )
