...
def tool_populate_options(parser):
    parser.add_option(
        ...
        "--machine-type",
        action="store",
        help="wpc, wpc95, stermSAM, sternWhitestar or custom (default)",
    )
    parser.add_option(
        "-r", "--repeat", action="store_true", help="Repeat the animation indefinitely"
    )
    ...
    parser.add_option(
        ...
        default=(128, 32),
        help="set the WIDTH and HEIGHT of the player (in dots)",
    )
...
def tool_run(options, args):
    ...
    game = PlayerGame(
        machine_type=machine_type, width=options.size[0], height=options.size[1]
    )
