...
def check_params(fpga_base, input_path, output_path):
    ...
    if len(fpga_base_list) != 2:
        return 0
    if not fpga_base_list[0].isdigit() or not fpga_base_list[1].isdigit():
        return 0

    # Make sure input path is a .dmd filename
    if not input_path.endswith(".dmd"):
        return 0
    if not os.path.isfile(input_path):
        ...
        anim = dmd.Animation()
        anim.populate_from_dmd_file(StringIO(f.read()))
        frame = anim.frames[0]
    ...
    if not output_path.endswith(".p-roc"):
        return 0
