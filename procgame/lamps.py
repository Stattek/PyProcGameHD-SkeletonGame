...
class LampShowTrack:
    ...
    def load_from_line(self, line):
        ...
        data = m.group("data") + (
            " " * 32
        )  # Pad it with 32 spaces so that the FIXME below doesn't cause a problem.
...
class LampShow:
    ...
    def tick(self):
        ...
        if time_diff > 0.500:
            ...
            for tr in self.tracks:
                if tr.driver is None:  # Lazily set drivers.
...
class LampController:
    ...
    def save_state(self, key):
        ...
        for lamp in self.game.lamps:
            state_dict[lamp.name] = {
                "time": lamp.last_time_changed,
                "state": lamp.state(),
            }
        ...
        if key in self.saved_state_dicts:
            ...
            for lamp_name, record in state_dict.iteritems():
                ...
                if lamp_name.find("gi0", 0) == -1:
                    time_remaining = (
                        record["state"]["outputDriveTime"] + record["time"]
                    ...
                    if (record["time"] == 0 or time_remaining < 1.0) and record[
                        "state"
                    ]["outputDriveTime"] != 0:
                    ...
                    # Otherwise, resume the lamp
                        ...
                        if record["state"]["timeslots"] == 0:
                            ...
                            self.game.lamps[lamp_name].schedule(
                                record["state"]["timeslots"],
                                duration,
