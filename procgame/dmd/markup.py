...
class MarkupFrameGenerator:
    ...
    def set_plain_font(
        self, font, interior_color=None, border_width=None, border_color=None
    ):
        self.font_plain = font
        if (
            interior_color is not None
        ...
        for draw in [False, True]:
            ...
            for line in lines:
                if line.startswith("{") and line.endswith("}"):  # frame!!
                    ...
                    y = self.__draw_text(
                        y=y, text=line, font=self.font_plain, justify="left", draw=draw
                    )
    ...
    def __draw_text(self, y, text, font, justify, draw):
        if max(font.char_widths) * len(text) > self.width:
            ...
            for ch in text:
                ...
                if w > self.width:
                    ...
                    if idx == -1:
                        ...
                        y = self.__draw_line(
                            y=y, text=line[:-1], font=font, justify=justify, draw=draw
                        )
                        line = ch
                    else:
                        # We have found a space!
                        y = self.__draw_line(
                            y=y, text=line[:idx], font=font, justify=justify, draw=draw
                        )
                        line = line[idx + 1 :]
                    # Recalculate w.
                    w = font.size(line)[0]
            if len(line) > 0:  # leftover text we need to draw
                y = self.__draw_line(
                    y=y, text=line, font=font, justify=justify, draw=draw
                )
            return y
        else:
            return self.__draw_line(
                y=y, text=text, font=font, justify=justify, draw=draw
            )
