...
class Particle:
    def __init__(self, x, y, emitter):
        ...
        self.life = random.randint(
            int(self.parent.max_life * 0.80), self.parent.max_life
        )
    ...
    def update_appearance(self):
        if self.life < 0.8 * self.parent.max_life:
            self.b = 0
...
class SnowParticle(Particle):
    def __init__(self, x, y, emitter):
        ...
        super().__init__(self.x, y, emitter)
        self.r = 225
        self.g = 225
        self.b = 255
...
class FireworkParticle(Particle):
    ...
    def update_location(self):
        if self.life < 0.75 * self.parent.max_life:
...
class ParticleEmitter:
    def __init__(
        ...
        for i in range(particles_per_update):
        ...
        sprImg16 = sdl2_DisplayManager.inst().load_surface(
            os.path.join(cwd, "exp16.png")
        )
        ...
        if self.creations_remaining <= 0:
            if self.random_next:
                if len(self.particles) == 0:
                    self.reset(
                        new_x=random.randint(0, 200), new_y=random.randint(0, 200)
                    )
            return

        for r in range(
            min(
                self.particles_per_update,
                self.max_particles - len(self.particles),
                self.creations_remaining,
            )
        ):
        ...
        for x in xrange(0, len(self.particles)):  # xrange(len(self.particles)-1,0,-1):
            ...
            if destination_texture is None:
                sdl2_DisplayManager.inst().screen_blit(
                    tx, x=p.x, y=p.y, expand_to_fill=False
                )
            else:
                sdl2_DisplayManager.inst().blit(
                    source_tx=tx,
                    dest_tx=destination_texture,
                    dest=(p.x, p.y, self.p_w, self.p_h),
                )
...
class ParticleLayer(Layer):
    ...
    def __init__(self, width, height, emitters, duration=None, num_hold_frames=1):
        ...
        self.ps = ParticleSystem(
            emitters=emitters, destination_texture=self.buffer.pySurface
        )
    ...
    def next_frame(self):
        ...
        elif self.duration is not None and (
            self.start_time + self.duration > time.time()
        ):
            return None
...
def main():
    ...
    ps1 = ParticleEmitter(
        ...
        max_life=20,
        max_particles=200,
        particles_per_update=40,
        total_creations=None,
        particle_class=FireParticle,
    )
    ...
    ps2 = ParticleEmitter(
        ...
        max_life=20,
        max_particles=300,
        particles_per_update=100,
        total_creations=300,
        particle_class=FireworkParticle,
        random_next=True,
    )
