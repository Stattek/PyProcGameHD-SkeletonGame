...
try:
    ...
    logging.error(
        "OpenCV is not available on your system.  The MovieLayer (mp4) support is unavailable"
    )
    OpenCV_avail = False
    ...
    def next_frame(self):
        ...
        if self.frame_sequence:
            ...
            if len(self.frame_sequence) > 1 and self.frame_time_counter == 0:
                if self.frame_sequence_pointer == (
...
class MovieLayer(Layer):
    ...
    def __init__(
        ...
        if movie is None and movie_file_path is None:
            raise ValueError(
                "MovieLayer requires either a movie_file_path argument -or- an instantiated movie object"
            )
    ...
    def next_frame(self):
        ...
        if (
            self.frame_pointer >= self.movie.frame_count
        ) and self.frame_time_counter == 0:
            if self.repeat:
                self.frame_pointer = 0
                self.movie.vc.set(capPropId("POS_FRAMES"), 0)
            elif self.hold:
        ...
        if self.frame_time_counter == 0:
            ...
            if rval and video_frame is not None:
...
class FrameQueueLayer(Layer):
    ...
    def next_frame(self):
        ...
        self.frame_time_counter -= 1
        if (not self.hold or len(self.frames) > 1) and (self.frame_time_counter == 0):
            if self.repeat:
                f = self.frames[0]
                del self.frames[0]
                self.frames += [f]
...
class TextLayer(Layer):
        ...
        if text is None:
            ...
            if (
                ...
                self.frame.fill_rect(
                    0, 0, self.width, self.height, self.fill_color
                )  # but taking this away shouldn't break it should it??
    ...
    def next_frame(self):
        ...
        if self.seconds is not None and (
            (self.started_at + self.seconds) < time.time()
        ):
            self.frame = None
...
class ScriptedLayer(Layer):
    ...
    **Script Format**
    ...
    def next_frame(self):
        ...
        if self.force_direction is not None or time_on_frame > script_item["seconds"]:
    ...
    def regenerate(self):
        ...
        for layer_item in self.script:
            if layer_item["layer"] is not None and hasattr(
...
class ScriptlessLayer(ScriptedLayer):
    ...
    def append(self, layer, seconds=None, callback=None):
        ...
        if callback is None:
            ...
            self.script.append(
                {"layer": layer, "seconds": seconds, "callback": callback}
            )
...
class ScoresLayer(ScriptlessLayer):
    ...
    def regenerate(self):
        ...
        for rec in self.game.get_highscore_data():
            ...
            lT = self.game.dmdHelper.genMsgFrame(
                records, self.background, font_key=self.fnt, font_style=self.font_style
            )
...
class GroupedLayer(Layer):
    ...
    def __init__(
        self, width=None, height=None, layers=None, fill_color=None, opaque=False
    ):
        """size is auto-detected from layers if omitted"""
        if width is None or height is None:
            if layers is None:
                raise ValueError(
                    "Cannot create an unsized grouped layer with no contents!"
                )
            if width is None:
...
class PanningLayer_pysurf(Layer):
    ...
    def __init__(
        ...
        translate,
        bounce=True,
        numFramesDrawnBetweenMovementUpdate=3,
    ):
        ...
        Frame.copy_rect(
            ...
            src_x=0,
            src_y=0,
            width=self.src_frame.width,
            height=self.src_frame.height,
        )
    ...
    def next_frame(self):
        ...
        if self.bounce:
            if self.translate[0] > 0 and (
                self.origin[0] + self.src_frame.width + self.translate[0] > self.width
            ):
                ...
                self.translate = (self.translate[0] * -1, self.translate[1])
                # print("hit left side")
            if self.translate[1] > 0 and (
                self.origin[1] + self.src_frame.height + self.translate[1] > self.height
            ):
                ...
                # print("hit top")
        self.origin = (
            self.origin[0] + self.translate[0],
            self.origin[1] + self.translate[1],
        )
...
class PanningLayer(Layer):
    ...
    def next_frame(self):
        ...
        if self.tick % self.holdFrames == 0:
            if self.bounce:
                if self.translate[0] < 0 and (
                    -self.origin[0] + frame.width + self.translate[0] > self.width
                ):
                    self.translate = (self.translate[0] * -1, self.translate[1])
                    # print("hit right side")
                elif self.translate[0] > 0 and (
                    -self.origin[0] + self.translate[0] < 0
                ):
                    self.translate = (self.translate[0] * -1, self.translate[1])
                    # print("hit left side")
                if self.translate[1] < 0 and (
                    -self.origin[1] + frame.height + self.translate[1] > self.height
                ):
                    self.translate = (self.translate[0], self.translate[1] * -1)
                    # print("hit top")
                elif self.translate[1] > 0 and (
                    -self.origin[1] + self.translate[1] < 0
                ):
                    self.translate = (self.translate[0], self.translate[1] * -1)
                    # print("hit bottom")
...
class RotationLayer(Layer):
    def __init__(
        self, x, y, rotation_per_update, content_layer, opaque=False, fill_color=None
    ):
        self.x = x
        self.y = y
        self.rotation = 0
    ...
    def next_frame(self):
        ...
        sdl2_DisplayManager.inst().roto_blit(
            self.tmp.pySurface, self.buffer.pySurface, dest, angle=self.rotation
        )
...
class ZoomingLayer(Layer):
    ...
    def next_frame(self):
        if self.total_zoomed > self.total_zooms and not self.hold:
            return None
...
class HDTextLayer(TextLayer):
    ...
    def set_text(
        self, text, seconds=None, blink_frames=None, style=None, force_update=False
    ):
        """Displays the given message for the given number of seconds."""
        ...
        if text is None or text == "":
            ...
            if fill_color is not None:
                ...
                x, y = (
                    (wOfText - self.width) * x_offset,
                    (hOfText - self.height) * y_offset,
                )

                self.font.drawHD(
                    self.frame, text, x, y, line_color, line_width, interior_color, None
                )
...
class AnimatedHDTextLayer(Layer):
    ...
    def next_frame(self):
        ...
        if self.line_width > 0:
            ...
            sdl2_DisplayManager.inst().blit(
                source_tx=edge, dest_tx=interior, dest=(0, 0, w, h)
            )
            w, h = edge.size
        ...
        if self.bg_color is not None:
            ...
            x, y = (
                int((wOfText - self.width) * x_offset),
                int((hOfText - self.height) * y_offset),
            )

            # self.font.drawHD(self.frame, text, x, y, line_color, line_width, interior_color, fill_color)
            sdl2_DisplayManager.inst().blit(
                source_tx=interior,
                dest_tx=self.frmAssembledResult.pySurface,
                dest=(x, y, w, h),
            )
    ...
    def set_text(self, text, seconds=None, blink_frames=None):
        if text != self.strLastMessage:
            ...
            (
                self.texTextBorder,
                self.texTextInterior,
            ) = sdl2_DisplayManager.inst().font_render_bordered_text_dual(
                text,
                font_alias=self.font.name,
...
def main():
    ...
    self.layer = dmd.AnimatedTextLayer(
        self.game.dmd.width / 2, self.game.dmd.height / 2, f, "center", 2
    )
