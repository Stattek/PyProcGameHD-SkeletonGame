...
class AnimationCacheManager:
        ...
        if not result:
            ...
            if version == DATABASE_VERSION:
                ...
                logging.getLogger("game.dmdcache").warning(
                    "DMD cache database version (%d) is not current (%d).  Cache will be rebuilt.",
                    version,
                    DATABASE_VERSION,
                )
                del self.conn
    ...
    def set_at_path(self, path, data):
        ...
        self.conn.execute(
            ...
            (path, created, accessed, "bzip2", sqlite3.Binary(data)),
        )
        self.conn.commit()
...
class Animation:
    ...
    def load(self, filename, allow_cache=True):
        ...
        *filename* can be a string or a list.  If it is a list, the images pointed
        ...
        if len(paths) == 0:
            raise ValueError(
                "Load FAILED: could not locate a file matching [%s]" % filename
            )
        key_path = paths[0]
        ...
        if data:
            ...
            for path in paths:
                ...
                if ext == ".dmd":
                    ...
                    self.populate_from_dmd_file(
                        StringIO(data), composite_op=composite_op
                    )
                    ...
                    if (
                        not animation_cache
                        and not warned_cache_disabled
                        and allow_cache
                    ):
                        logger.warning(
                            "Loading image file with caching disabled; set dmd_cache_path in config to enable."
                        )
                    ...
                    with open(path, "rb") as f:
                        self.populate_from_image_file_sdl2(
                            path, f, composite_op=composite_op
                        )
    ...
    def save(self, filename):
        ...
        if self.width is None or self.height is None:
            raise ValueError(
                "width and height must be set on Animation before it can be saved."
            )
    ...
    def populate_from_image_file(self, path, f, composite_op=None):
        if not Image:
            raise RuntimeError(
                "Cannot open non-native image types without Python Imaging Library: %s"
                % (path)
            )
        ...
        if len(self.frames) > 0 and (w != self.width or h != self.height):
            raise ValueError(
                "Image sizes must be uniform!  Anim is %dx%d, image is %dx%d"
    ...
    def populate_from_dmd_file(self, f, composite_op=None):
        ...
        if dmd_style == 0:
            if file_length != 16 + self.width * self.height * frame_count:
                ...
                raise ValueError(
                    "File size inconsistent with original DMD format header information.  Old or incompatible file format?"
                )
        elif dmd_style == 1:
            if file_length != 16 + self.width * self.height * frame_count * 3:
                logging.getLogger("game.dmdcache").warning(f)
                raise ValueError(
                    "File size inconsistent with true-color DMD format header information. Old or incompatible file format?"
                )
        ...
        for frame_index in range(frame_count):
            ...
            if dmd_style == 0:
                ...
                new_frame.pySurface = (
                    sdl2_DisplayManager.inst().make_texture_from_imagebits(
                        bits=str_frame,
                        width=self.width,
                        height=self.height,
                        composite_op=composite_op,
    ...
    def save_to_old_dmd_file(self, f):
        header = struct.pack(
            "IIII", 0x00646D64, len(self.frames), self.width, self.height
        )
        if len(header) != 16:
    ...
    def save_to_dmd_file(self, f):
        header = struct.pack(
            "IIII", 0x00DEFACE, len(self.frames), self.width, self.height
        )
        if len(header) != 16:
            raise ValueError("Packed size not 16 bytes as expected: %d" % (len(header)))
        f.write(header)
