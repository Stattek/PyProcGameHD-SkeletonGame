...
class LayerTransitionBase:
    ...
    def next_frame(self, from_frame, to_frame):
        ...
        self.progress = max(
            0.0, min(1.0, self.progress + self.progress_mult * self.progress_per_frame)
        )
...
class WipeTransition(LayerTransitionBase):
    ...
    def transition_frame(self, from_frame, to_frame):
        ...
        Frame.copy_rect(
            ...
            src_x=src_x,
            src_y=src_y,
            width=from_frame.width - src_x,
            height=from_frame.height - src_y,
            op="copy",
        )
        return frame
    ...
    def transition_frame(self, from_frame, to_frame):
        ...
        src_x, src_y, ovr_x, ovr_y = {
            "north": (
                ...
                prog1 * frame.height,
                0,
                frame.height - prog0 * (self.obs_frame.height + 2 * frame.height),
            ),
            "south": (
                0,
                prog0 * frame.height,
                0,
                frame.height - prog1 * (self.obs_frame.height + 2 * frame.height),
            ),
            "east": (
                prog0 * frame.width,
                0,
                frame.width - prog1 * (self.obs_frame.width + 2 * frame.width),
                0,
            ),
            "west": (
                prog1 * frame.width,
                0,
                frame.width - prog0 * (self.obs_frame.width + 2 * frame.width),
                0,
            ),
        }[self.direction]
        if self.direction in ["east", "south"]:
        ...
        Frame.copy_rect(
            ...
            src_x=0,
            src_y=0,
            width=self.obs_frame.width,
            height=self.obs_frame.height,
...
class CrossFadeTransition(LayerTransitionBase):
    ...
    def transition_frame(self, from_frame, to_frame):
        ...
        sdl2.SDL_SetTextureAlphaMod(
            from_frame.pySurface.texture, int(255 - alpha_value)
        )
