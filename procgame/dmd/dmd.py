...
class Frame(object):
    ...
    def __init__(self, width, height, from_surface=None):
        ...
        self.x_offset: int = 4  # bytes_per_pixel
        self.y_offset: int = self.width * 4  # every y_offset Bytes is the next line
        self.font_dots = []
        ...
        for row_index in range(0, num_rows):
            for col_index in range(0, num_cols):
                ...
                Frame.copy_rect(
                    ...
                    dst_y=0,
                    src=self,
                    src_x=width * col_index,
                    src_y=height * row_index,
                    width=width,
                    height=height,
                    op="copy",
                )
    ...
    def set_alpha(self, value=255):
        print("SETTING ALPHA VALUE TO : ", value)
    ...
    def build_surface_from_8bit_dmd_string(self, str_data, composite_op=None):
        ...
        self.pySurface = sdl2_DisplayManager.inst().make_texture_from_imagebits(
            bits=d,
            width=self.width,
            height=self.height,
            mode="RGB",
...
class Layer(object):
    ...
    composite_op = "copy"
    """DEPRECATED: Composite operation used by :meth:`composite_next` when calling :meth:`~pinproc.DMDBuffer.copy_rect`."""
    transition = None
    ...
    def composite_next(self, target):
        ...
        if src is not None:
            ...
            Frame.copy_rect(
                dst=target,
                dst_x=self.target_x + self.target_x_offset,
                dst_y=self.target_y + self.target_y_offset,
                src=src,
                src_x=0,
                src_y=0,
                width=src.width,
