...
try:
    ...
    if os.getenv("PYSDL2_DLL_PATH") is None:
        ...
        if p is None:
            print(
                "Tried to load SDL2 but failed.  Please set PYSDL2_DLL_PATH in your system environment variables or in your config.yaml"
            )
            import sys
            ...
            try:
                ...
                print(
                    "Tried to load SDL2 but failed. \nThe PYSDL2_DLL_PATH in your config.yaml ['%s'] does contain a valid SDL2 DLL (or SDL2 is not properly installed on your system)"
                    % p
                )
                print(e2)
                import sys
    ...
    def render_border(
        ...
        **kwargs,
        ...
        if border_color is None:
            raise ValueError(
                "Border color should not be None when calling render_border"
            )
...
class sdl2_DisplayManager:
    def __init__(
        ...
        self.window = sdl2.ext.Window(
            title, size=(self.window_w, self.window_h), flags=flags
        )
    ...
    def fonts_init(
        ...
        if self.font_manager is None:
            self.font_manager = FontManagerExtended(
                font_path=font_path, alias=default_font_alias, size=size
            )
        else:
            raise ValueError(
                "sdl2_DisplayManager: fonts_init already completed previously.  Don't do it again."
            )

    def font_add(
        self, font_path, font_alias, size=None, color=None, bgcolor=None, bold=False
    ):
    ...
    def font_render_text(
        self, msg, font_alias=None, size=None, width=None, color=None, bg_color=None
    ):
        # color = (color[0], color[1], color[2], 255)
        tsurf = self.font_manager.render(
            msg, alias=font_alias, size=size, width=None, color=color, bg_color=bg_color
        )
    ...
    def font_render_bordered_text_dual(
        self,
        msg,
        font_alias=None,
        size=None,
        ...
        bsurf, isurf = self.font_manager.render_border(
            msg,
            alias=font_alias,
            size=size,
            width=width,
    ...
    def mask(self, txA, txB):
        ...
        ret = sdl2.SDL_SetTextureBlendMode(
            txB.texture, sdl2.SDL_BLENDMODE_MOD
        )  # apply mod to 'source'
        if ret == -1:
            raise sdl2.ext.SDLError()
    ...
    def combine(self, txA, txB, txC):
        ...
        ret = sdl2.SDL_SetTextureBlendMode(
            txC.texture, sdl2.SDL_BLENDMODE_MOD
        )  # apply mod to 'source'
        if ret == -1:
            raise sdl2.ext.SDLError()
    ...
    def font_render_bordered_text_Faster(
        self,
        txtarget,
        dst_loc,
        msg,
        ...
        srf_outline, srf_interior = self.font_manager.render_border(
            msg,
            alias=font_alias,
            size=size,
            width=width,
        ...
        self.blit(
            tx_outline,
            txtarget,
            (dst_loc["x"] + 0, dst_loc["y"] + 0, srf_outline.w, srf_outline.h),
        )
        self.blit(
            tx_interior,
            txtarget,
            (
                dst_loc["x"] + border_width,
                dst_loc["y"] + border_width,
                srf_interior.w,
                srf_interior.h,
            ),
        )
    ...
    def blit(
        ...
        if blendmode is not None:
            if blendmode == "ADD":
                ret = sdl2.SDL_SetTextureBlendMode(
                    source_tx.texture, sdl2.SDL_BLENDMODE_ADD
                )
            elif blendmode == "MOD":
                ret = sdl2.SDL_SetTextureBlendMode(
                    source_tx.texture, sdl2.SDL_BLENDMODE_MOD
                )
            elif blendmode == "BLEND":
                ret = sdl2.SDL_SetTextureBlendMode(
                    source_tx.texture, sdl2.SDL_BLENDMODE_BLEND
                )
            else:
                ret = sdl2.SDL_SetTextureBlendMode(
                    source_tx.texture, sdl2.SDL_BLENDMODE_NONE
                )
            if ret == -1:
                raise sdl2.ext.SDLError()
    ...
    def roto_blit(
        self, source_tx, dest_tx, dest, area=None, angle=0, origin=None, flip=0
    ):
        """a blit function, backed by RenderCopy, that emulates PyGame 1.9.2 style blitting
        def blit(source_tx, dest_tx, area=None, special_flags = 0)
    ...
    def _render_copy_ex(
        self, src, srcrect=None, dstrect=None, angle=0, origin=None, flip=0
    ):
        ...
        ret = sdl2.render.SDL_RenderCopyEx(
            rdr, texture, srcrect, dstrect, angle, origin, flip
        )
        ...
        if (not _HASSDLIMAGE) or (composite_op is not None and composite_op != "None"):
            ...
            if composite_op is not None and composite_op != "None":
                if composite_op == "blacksrc":
                    ...
                    raise ValueError(
                        "Composite_op '%s' not recognized/supported." % composite_op
                    )
                ...
                if sf.format.contents.BytesPerPixel == 4:
                    trans_color = sdl2.pixels.SDL_MapRGBA(
                        fmt, trans_color.r, trans_color.g, trans_color.b, trans_color.a
                    )
                else:
    ...
    def make_texture_from_imagebits(
        self, width, height, bits, mode="RGB", composite_op=None
    ):
        ...
        tsurface = sdl2.surface.SDL_CreateRGBSurfaceFrom(
            bits, width, height, depth, pitch, rmask, gmask, bmask, amask
        )
        if not tsurface:
            raise sdl2.ext.SDLError()
    ...
    def make_texture_from_bits(self, bits, width, height):
        ...
        rmask = gmask = bmask = amask = 0
        imgsurface = sdl2.surface.SDL_CreateRGBSurface(
    ...
    def new_texture(self, width, height, color=(0, 0, 0, 0)):
        t = sdl2.render.SDL_CreateTexture(
            self.texture_renderer.renderer,
    ...
    def texture_from_surface(self, surface):
        ...
        if isinstance(surface, sdl2.ext.SoftwareSprite):
            texture = sdl2.render.SDL_CreateTextureFromSurface(
                self.texture_renderer.renderer, surface.surface
            )
        else:
            texture = sdl2.render.SDL_CreateTextureFromSurface(
                self.texture_renderer.renderer, surface
            )
        if not texture:
            raise sdl2.ext.SDLError()
        t = sdl2.ext.TextureSprite(texture.contents)
...
def do_something(disp_mgr, i, tx_list, diff, tx_sprite):
    if i <= 150:
        ...
        disp_mgr.blit(
            source_tx=tx_sprite, dest_tx=tx2, dest=(i * 5, i, 0, 0)
        )  # , area=(0,0,50,50))
        ...
        disp_mgr.screen_blit(
            source_tx=tx2, expand_to_fill=True
        )  # , area=(10,10,400,200))
        ...
        A, B = disp_mgr.font_render_bordered_text_dual(
            "Took: % 3.10fms" % diff,
            font_alias="HHSam_30",
            size=80,
            width=None,
        ...
        disp_mgr.screen_blit(
            source_tx=tx_tmp, expand_to_fill=True
        )  # , area=(10,10,400,200))
...
def main():
    ...
    disp_mgr.font_add(
        font_path="assets/T2.ttf", font_alias="T2_30", size=30
    )  # , color=None, bgcolor=None)
    f = disp_mgr.font_add(
        font_path="assets/HH_Samuel.ttf", font_alias="HHSam_30", size=30
    )  # , color=None, bgcolor=None)
