...
class HDFont:
    ...
    def __init__(self, fontname, size, bold=False, font_file_path=None):
        ...
        try:
            p = sdl2_DisplayManager.inst().font_add(
                font_path=font_path,
                font_alias=fontname,
                size=None,
                color=None,
        ...
        self.font_width, self.font_height = sdl2_DisplayManager.inst().font_get_size(
    ...
    def draw_in_rect(
        self, frame, text, rect=(0, 0, 128, 32), anchor=AnchorCenter, font_size=None
    ):
...
def main():
    ...
    sdl2_DisplayManager.inst().font_add(
        font_path=font_path, font_alias="export_font", size=font_size
    ...
    sdl2_DisplayManager.inst().window = sdl2.ext.Window(
        "Font Preview", size=(width, height)
    )
    ...
    imgsurface = sdl2.surface.SDL_CreateRGBSurfaceFrom(
        bytes, width, height, 32, pitch, rmask, gmask, bmask, amask
    )
    if not imgsurface:
