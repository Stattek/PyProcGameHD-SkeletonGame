...
class Font:
        ...
        if ext == ".dmd":
            ...
            if char_widths is None:
                for i in range(96):
                    ...
                    self.char_widths += [
                        self.__anim.frames[1].get_dot(
                            i % self.__anim.width, i / self.__anim.width
                        )
                    ]
            ...
            raise ValueError(
                "Font failed to load [%s] Expected font of type .dmd, .png, .bmp, or .jpg"
                % filename
            )
        return self
    ...
    def save(self, filename):
        ...
        for i in range(96):
            out.frames[1].set_font_dot(
                i % self.__anim.width, i / self.__anim.width, self.char_widths[i]
            )
...
class AnimFont(Font):
    ...
    def load(self, filename):
        ...
        if len(self.__anim.frames) < 1:
            raise ValueError(
                "Expected a minimum 2 frames: %d" % (len(self.__anim.frames))
            )
        ...
        for i in range(96):
            ch = chr(int(i + ord(" ")))
            self.char_widths += [
                self.char_data["positions"][ch]["Base Width"]
                + self.char_data["positions"][ch]["Width Offset"]
            ]
    ...
    def draw(self, frame, text, x, y):
        ...
        for ch in text:
            ...
            if char_offset < 0 or char_offset >= 96:
                continue
            char_x = (
                self.char_width * (char_offset % 10)
                + self.char_data["positions"][ch]["X Offset"]
            )
            char_y = (
                self.char_height * (char_offset / 10)
                + self.char_data["positions"][ch]["Y Offset"]
            )
            width = self.char_widths[char_offset]
...
def init_font_path():
    ...
    try:
        ...
        if issubclass(type(value), list):
            ...
            print(
                "ERROR loading font_path from %s; type is %s but should be list or str."
...
def font_named(name):
    ...
    if path:
        from . import dmd  # have to do this to get dmd.Font to work below... odd.
        ...
        if font_path is None:
            raise ValueError(
                'Failed to load font "%s"; font_path was not set in your config.yaml (or config.yaml contains errors)'
                % (name)
            )
        raise ValueError(
            'Font named "%s" not found; font_path=%s.  Have you configured font_path in config.yaml?'
            % (name, font_path)
        )
