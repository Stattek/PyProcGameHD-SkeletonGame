    ...
    **Using DisplayController**
    ...
    def __init__(self, game, width=192, height=96, message_font=None):
        ...
        if message_font is not None:
            self.message_layer = TextLayer(
                width / 2, height - 2 * 7, message_font, "center"
            )
    ...
    def set_message(self, message, seconds):
        if self.message_layer is None:
            raise ValueError(
                "Message_font must be specified in constructor to enable message layer."
            )
    ...
    def update(self):
        ...
        for mode in self.game.modes.modes:
            if hasattr(mode, "layer") and mode.layer is not None and mode.layer.enabled:
        ...
        # We reverse the list here so that the top layer gets the last say.
        for layer in layers[::-1]:
            if layer.enabled:
                layer.composite_next(self.frame)

        if self.message_layer is not None:
